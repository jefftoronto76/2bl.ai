...
| `services/chat/server/index.ts` | Public orchestrator `streamChat()` — normalizes messages (empty → greeting `'Hi'`; leading-assistant gets `'Hi'` prepended). Since Phase 3a, `streamChat` calls `resolveTurnPrompt` (`services/chat/server/turn-context/`) and passes `resolved.system` to `runChatStream`. The six-segment concatenation has been deleted. Runs one streamed turn, returns the data-stream Response (502 on upstream error). Re-exports the public contract types. |
