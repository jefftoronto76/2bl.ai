...
**SQL run:** ⚠️ _Inferred from `docs/auth-service-rebuild.md` §7 and the
column's behavior in code — confirm from Studio:_
```sql
...
**SQL run:** ⚠️ _Inferred from the webhook write path (nullable timestamptz,
no default) — confirm column definition from Studio:_
```sql
