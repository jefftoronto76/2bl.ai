...
// ── Shared V2 domain types (imported by the modals) ─────────────────────────
...
function SectionLabel({
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span className={`font-mono tracking-[0.2em] uppercase text-text-muted ${large ? 'text-sm' : 'text-[11px]'}`}>
        {children}
      </span>
    </div>
  );
...
const MENU_ITEMS: { key: RowAction; icon: typeof Star; label: string; danger?: boolean }[] = [
  ...
  { key: 'moveToChapter', icon: FolderInput, label: 'Move to story' },
  { key: 'removeFromChapter', icon: FolderMinus, label: 'Remove from story' },
    ...
    >
      ...
      {expanded && (
        <div className="flex flex-col gap-6 mt-5 px-3 pb-3 flex-1 min-h-0 overflow-y-auto">
          {/* Stories */}
          <div>
            <SectionLabel icon={BookOpen} large>Stories</SectionLabel>
