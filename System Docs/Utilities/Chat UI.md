...
**Workspace sizing (2026-09).** `ChatDrawerV2` provides `v2/WorkspaceContext.tsx`. Descendants call `useWorkspaceWidthRequest('navExpanded' | 'landscapePreview', active)` to make the drawer grow instead of squeezing its content. `useWorkspace()` exposes the drawer's own `isFullScreen`/`onToggleFullScreen`. Heirloom's widths are 672px (`max-w-2xl`) with the Nav on its rail and 880px (`navExpandedWidthClassName`) with it expanded; Preview's Landscape page adds a min-width, and full screen ignores both requests. The hooks are no-ops outside a provider. Details and trade-offs are in `System Docs/Known Gaps.md`, "Workspace sizing".
