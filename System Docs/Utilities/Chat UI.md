...
| `useWidgetShell.ts` | `useWidgetShell` | The jefflougheed widget-shell presentation store — a headless module-level Zustand singleton (no JSX) owning **shell** state only: `isExpanded`, `expand(mode?)`, `collapse`, `mode`, `setMode`, `composerRef`, `setComposerRef`, `heroEngaged`, `setHeroEngaged` (consumed by `SectionRail.tsx`). Conversation state (messages/sessionId/streaming/error/mode) lives in the shared session (`useChatSession`, instanceKey `"sage"`) — NOT here. Being a module singleton is load-bearing: the overlay (`Chat`), the inline `Hero` composer, and `SectionProcess` read/write the same object, so opening the overlay from one surface is seen by the others. `expand('question')` sets `isExpanded` + `mode` together — the pairing Chat's mode-bridge depends on. Extracted from `src/lib/store.ts` (`useSageStore`) in centralization Step E; the conversation slice had already migrated to `useChatSession`, and five callerless fields (`visitorName`/`setVisitorName`, `hasGreeted`/`setGreeted`, `focusComposer`) plus the callerless `reset()` were dropped. Unit-tested in `useWidgetShell.test.ts`. |
| `useChatTurn.ts` | `useChatTurn` | Store-agnostic turn engine (`'use client'`). Takes injected `ChatEngineAccessors` (`getMessages` / `addMessage` / `updateLastMessage` / `patchMessageById` / `removeMessageById` / `truncateAfter` / `setStreaming` / `setSessionId` / `getSessionId` / `getMode?` / `getMemberId?` / `getInviteToken?` / `getMediaItems?` / `markMediaItemsDelivered?` / `getSessionContextToAttach?`) and owns one turn end-to-end: append user message → lazily create a session (`POST /api/sessions`, optionally carrying `mediaItemIds` and/or `contextType`/`contextRefId`/`contextFrequency` read from `getMediaItems?`/`getSessionContextToAttach?` at that exact moment — session-context-service, PR #383, see `System Docs/Utilities/Chat Server.md`) → stream from `/api/sage` (via the shared `readDataStream`) → persist the transcript (`PATCH /api/sessions/[id]`, `visitorName: null`). Returns `{ send, sendHidden, retry, stop, regenerate, setActiveVersion, editMessage, resendMessage, isStreaming, errorType }`. `errorType` classifies why the most recent turn didn't complete normally (`ChatErrorType`: `network` / `rate_limited` / `stream_interrupted` / `auth_error` / `unknown` / `user_stopped` — the 6 this hook's own classifier can produce; `services/chat/ui/v1/types.ts`'s union also has 3 more, `account_required` / `server_error` / `invalid_response`, added 2026-08-06 for the memory-bookmark create path below and never produced here or persisted to `chat_sessions.last_error_type`) — `null` when it succeeded. **`rate_limited`/`auth_error` have two distinct sources as of 2026-09-16 (PR #486):** a pre-stream HTTP status (429/401, unchanged) *or* a server-classified in-stream failure via `readDataStream`'s error callback → `classifyStreamFailure` (`./streamError.ts`, own row below) — before this, every post-200 failure collapsed to `stream_interrupted` regardless of cause. On a failure the hook tags the failed message (the user message for `send`/`retry`/`editMessage`/`resendMessage`, the assistant placeholder for `sendHidden`/`regenerate`) with `UIMessage.error_type` and PATCHes `/api/sessions/[id]` with `last_error_type`, so both the per-message and per-session classification reach the DB (previously the PATCH never fired on a failed turn). `user_stopped` (2026-07-28) goes through the identical classify → banner → persist path when the visitor hits Stop, rather than a special-cased silent branch — see "Stop / interrupted-turn protocol" below. Each surface's error banner renders the matching string from `components/chat/errorCopy.ts`. `editMessage(messageId, text)` / `resendMessage(messageId)` (added 2026-07-27) truncate the transcript forward from a visitor message (via `truncateAfter`) and re-deliver it — `editMessage` also replaces its content and sets `UIMessage.edited`; both share the `truncateAndRedeliver` internal, hard-cancel any in-flight turn synchronously before mutating (sequencing rule: a stream token arriving after truncation must never write into a message that no longer exists), and fire three fire-and-forget cleanup calls for whatever the truncation drops, none of which block the edit/resend flow on failure: `DELETE /api/sessions/[id]/feedback?fromIndex=N` (`services/crm/feedback.ts` `deleteFeedbackFrom`) so a new reply can never silently inherit a rating that belonged to different, now-discarded content; `PATCH /api/sessions/[id]/conversion-events?after=<cutoff timestamp>` to flip any still-`presented` conversion event (booking offer / contact capture) tied to a discarded reply to `overwritten` (`services/crm/conversion-events.ts`); and `DELETE /api/sessions/[id]/memories?messageIds=<dropped ids>` to hard-delete any memory anchored to one of the dropped messages (`services/crm/memories.ts`) — keyed by message id, not position, so unlike the feedback cleanup there's no index-cutoff math, just the ids `truncateAfter` is about to drop. Rendered by `components/chat/UserMessageActions.tsx` (Edit/Copy/Send again row) + `components/chat/EditableUserBubble.tsx` (the in-place editing textarea), wired into `components/shells/membership/MessageList.tsx`. jefflougheed (via `ChatSessionProvider`/`useChatSession`, `instanceKey="sage"`) and Heirloom (`useReducer` via `ChatProvider`) both consume it by wrapping their store in `ChatEngineAccessors`. |
| `streamError.ts` | `classifyStreamFailure` (+ `StreamFailure`) | Added 2026-09-16 (PR #486). Pure, no React, no fetch — pulled out of `streamTurn` specifically so the mapping is unit-testable on its own. Maps `readDataStream`'s in-stream error message (a `StreamErrorCode` from `services/chat/server/stream-utils.ts` when the server classified it via `describeStreamError`, see `System Docs/Utilities/Chat Server.md`'s stream-error section — `''` when it didn't, e.g. a pre-#486 deployment) onto `{ errorType: ChatErrorType, detail?: StreamErrorCode }`. Only `rate_limited`/`auth_error` map to anything other than `stream_interrupted`; `detail` is diagnostic only and never reaches `chat_sessions.last_error_type` — `errorType` is the sole value persisted, via `ChatTurnError` in `useChatTurn.ts`. |
| `useMessageFeedback.ts` | `useMessageFeedback` (+ `FeedbackRating`, `MessageFeedbackState`, `UseMessageFeedbackReturn`) | Headless `'use client'` data hook (no JSX) for the thumbs/reason-chip feedback captured by `components/chat/MessageActions.tsx` + `FeedbackPopover.tsx`. Fetches existing ratings for a session on mount (`GET /api/sessions/[id]/feedback`) and posts rating/reason/note changes (`POST`, same route) — mirrors `useSageParameters.ts`'s fetch-on-mount, resilient-to-errors pattern. Keyed by `message_index` — the message's position in the persisted `messages` array — **not** the client-side `UIMessage.id`, matching the `message_feedback` table's column; this is the deliberate contrast `useMemories.ts`'s own doc comment calls out (id-keyed to dodge the index-drift class of bug this hook has to guard against instead, e.g. `truncateAndRedeliver`'s `DELETE .../feedback?fromIndex=N` cleanup in `useChatTurn.ts`). Returns `{ getFeedback, rate, submitFeedback }`: `getFeedback(messageIndex)` returns `{ rating: null, tags: [], detail: '' }` for an unrated message; `rate(messageIndex, rating)` sets the rating and toggles it off if it's already active, persisting immediately; `submitFeedback(messageIndex, reasons, note)` persists the reason chips + note while keeping the current rating. All posts are fire-and-forget (`.catch(console.error)`), no retry/error state surfaced to the UI. |
| `useMemories.ts` | `useMemories` | Headless `'use client'` data hook (no JSX) for the Heirloom memory bookmark/card — fetch-on-mount, keyed by the anchor message's stable `UIMessage.id` (not array position, to avoid the index-drift class of bug `useMessageFeedback` guards against on edit/truncate). Returns `{ memories, getByAnchor, isPending, getPendingKind, hasError, getErrorType, hasOpenDraft, isLoaded, create, keep, discard, rename, reviseBlocks, assignToStory, removeFromStory }`. `MemoryRow` also carries `storyId?: string | null` — the story a memory currently belongs to (`null` = never assigned, `undefined` on a freshly-created/renamed/revised row, since those writes don't touch it). `assignToStory(memoryId, storyId)` (`MemoryCardView`'s "+" via `StoryPicker`) PATCHes `action: 'assign_story'` (`assignMemoryToStory`, `services/crm/story-containments.ts`); `removeFromStory(memoryId)` (StoryPicker's "Remove from '[Story]'" item, 2026-08-14) PATCHes `action: 'remove_story'` (`removeMemoryFromStory`, same file) with no `story_id` needed — it just clears whatever containment currently exists. Both mirror `rename()`'s trust model rather than `reviseBlocks()`'s: `storyId` is a field the client fully controls (the id it just picked, or removal), so both echo the new value (the picked id, or `null`) straight into local state on success rather than reading anything back from the response. `reviseBlocks(memoryId, blocks)` (Memory Canvas V1, 2026-08-08) PATCHes `action: 'revise_blocks'` and — unlike `rename()`, which trusts the argument it was given since `title` is a field the client fully controls — parses the response and merges the server's own `body_blocks`/`body` into local state, since `body` is derived server-side from `blocks` (`reviseMemoryBlocks`, `services/crm/memories.ts`); echoing the client's own argument here was a real bug found in review (fixed same day) that left `body` stale after every block edit. `hasOpenDraft(anchorId)` and `keepDisabled` (the manual bookmark's suppression flag, computed in `components/shells/membership/MessageList.tsx`) are **per-anchor** (fixed 2026-08-06, PR #288) — previously a single whole-session boolean, so one open draft anywhere disabled every bookmark in the transcript, including retries of a failed one (the exact regression `Design Handovers/heirloom_chat_handoffV2_2026_July 28/memories/README.md` §6 rule #3 warned against); now `hasOpenDraft(X)` is true only while anchor `X` itself has an open draft, so a stale draft on one message never blocks another's bookmark or its own retry. `create(anchorMessageId, sourceKind)` POSTs `/api/sessions/[id]/memories`; on failure it classifies the response via a module-local `classifyCreateFailure(res, data)` into a `ChatErrorType` (status `401` → `account_required`, `5xx` → `server_error`, any other non-ok → `unknown`, a `2xx` with a missing/malformed `memory` field → `invalid_response`; a thrown `fetch()` → `network`) and stores it per-anchor via `getErrorType(anchorId)` (also added 2026-08-06 — previously `erroredAnchors` was a plain per-anchor boolean with one hardcoded error message regardless of cause). `components/shells/membership/memory/MemoryCard.tsx`'s `MemoryErrorLine` renders `ERROR_COPY[errorType]` (`components/chat/errorCopy.ts`, same shared vocabulary `useChatTurn.ts`'s own error banner uses above) with a "Try again" button that re-invokes `create()` for that exact anchor — which only behaves correctly now that `hasOpenDraft`/`keepDisabled` are per-anchor, since a session-wide suppression flag would have kept the retry disabled regardless. No model call anywhere in this hook or in `services/crm/memories.ts` — `create()` is a verbatim write; see `System Docs/Utilities/CRM.md`'s `memories.ts` row for the server side (`user_id` resolution, the 401 rejection, and audit logging). **Caveat confirmed by a real bug, 2026-08-14 (PR #385):** the `memories` array itself is **not** cleared when `sessionId` changes — only `loadedForSessionId` (and therefore the derived `isLoaded`) resets in the effect below, by design (see this file's own doc comment on why `isLoaded` is derived rather than stored). A consumer that reads `memories` directly without also checking `isLoaded`/the active session id can render a just-abandoned session's rows after a session switch — this is exactly what happened to `ChatHero.tsx`'s new `SessionMemoriesPanel` pane before a filter fixed it; see `System Docs/Known Gaps.md`'s Memories entry for the incident and `System Docs/Public Site.md`'s `SessionMemoriesPanel` row for the fix. |
...
| `index.ts` | barrel | Re-exports the marker type contracts (`MarkerType`, `ParsedMarker`, `MarkerParseResult`, `MarkerDispatch`, `MarkerDefinition`, `MarkerRegistry`, `ChatEngineAccessors`, `UseChatTurnOptions`, `UseChatTurnReturn`, `UIMessage`, `AuthPromptData`, `ChatErrorType`) + the registry runtime (`createMarkerRegistry`, `createDefaultRegistry`, `BOOKING_MARKER`, `NAME_MARKER`, `EMAIL_MARKER`, `PHONE_MARKER`, `ACCOUNT_CREATE_MARKER`, `SAVE_MEMORY_MARKER`, `MEMORY_TITLE_MARKER`) + `message.ts`'s canonical helpers (`normalizeTimestamp`, `createUIMessage`, `reviveUIMessage`, `reviveUIMessages`, `toChatMessage`, `toChatMessages`, `toModelMessages` — pure and server-safe, unlike the hooks below) + the session-core type contracts (`ChatSessionState`, `ChatSessionStore`, `HydrateInput`, `ChatSession`, `ChatSessionConfig`). `useChatTurn` and `useChatSession` (both `'use client'`) are deliberately NOT re-exported — server consumers (e.g. the admin transcript renderer) need to import the registry without pulling in a client-only module; import the hooks directly from their own files. |

**Entry points into the Heirloom chat shell (added 2026-09-07, after the PR #468 lander rebuild).** `OPEN_CHAT` is produced by the lander's CTAs and consumed by `app/heirloom/HeirloomApp.tsx`, which mounts `ChatProvider`, renders `components/landing/LandingPage.tsx`, and layers `ChatDrawerV2` over it. Producers: `LandingNav.tsx` ("Start Your Story"), `HeroSection.tsx` twice (the desktop hero button and the ≤768px `MobileStoryThread`'s terminal button — only one is visible at any width), `PricingSection.tsx` ("Drop us a message") and `CtaSection.tsx` (via `PrimaryCta`). Every one is the literal `dispatch({ type: 'OPEN_CHAT' })` through `useChatStore`; the design reference's `legacy-open-chat` window CustomEvent was deliberately not ported. The shared button components in `LandingCta.tsx` (`PrimaryCta` / `GhostCta`) are visual-only and take a plain `onClick` — **the calling section owns the dispatch**, which is the invariant a future refactor is most likely to break. `HeirloomApp.tsx` also binds Escape to `CLOSE_CHAT` while the panel is open; the V2 modals register capture-phase Escape handlers with `stopPropagation`, so one press closes one layer. Lander detail lives in `System Docs/Public Site.md` ("Heirloom lander").

**Workspace sizing (2026-09).** `ChatDrawerV2` provides `v2/WorkspaceContext.tsx`. Descendants call `useWorkspaceWidthRequest('navExpanded' | 'landscapePreview', active)` to make the drawer grow instead of squeezing its content. `useWorkspace()` exposes the drawer's own `isFullScreen`/`onToggleFullScreen`, which `PreviewModal`'s full-screen button reuses. Heirloom's widths are 672px (`max-w-2xl`) with the Nav on its rail and 880px (`navExpandedWidthClassName`) with it expanded; Preview's Landscape page adds a min-width, and full screen ignores both requests. The hooks are no-ops outside a provider. Details and trade-offs are in `System Docs/Known Gaps.md`, "Workspace sizing".
...
| `core/useChatSession.ts` | `useChatSession`, `ChatSession`, `ChatSessionConfig` | Core hook: resolves the backing store (singleton if `instanceKey` provided, otherwise ref-local isolated), builds `ChatEngineAccessors` (including `truncateAfter`, added 2026-07-27, and `getSessionContextToAttach`, added 2026-08-14 — threaded through from `ChatSessionConfig` verbatim, same pattern as `getMemberId`/`getInviteToken`), calls `useChatTurn` once, exposes `send` / `sendHidden` / `retry` / `stop` / `regenerate` / `setActiveVersion` / `editMessage` / `resendMessage` / `setMode` / `hydrate` / `reset` plus the full `ChatSession` state. One provider → one engine. |
