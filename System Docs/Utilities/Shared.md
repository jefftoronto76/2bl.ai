...
| `escapeForTag` | `prompt-text.ts` | Escapes literal `<`/`>` in a value about to be interpolated into an XML-tag-delineated prompt block, so untrusted text can't break out of its wrapper. Moved here 2026-09-09 (Traffic Cop Phase 1) from `services/chat/server/session-context.ts`, which re-exports it unchanged, so the turn-context runner (`services/chat/server/turn-context/runner.ts`'s `delineate`) can apply the same escaping to every operator/participant-authored block without importing session-context's DB and story dependencies. Dependency-free. Covered by `session-context.test.ts`'s `escapeForTag` suite and `runner.test.ts`'s delineation suite. |
