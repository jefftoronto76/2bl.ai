...
- `chat_id` is nullable, though the spec's language implies it should always be set for anything uploaded through chat. Open question: is there a legitimate upload path with no chat context?
  **Resolved, 2026-08-06 (PR #291):** not a legitimate no-chat-context path — a real bug. `ChatInput.tsx` uploads attachments *before* calling `sendMessage()`, and `sendMessage()` is what lazily creates the session, so an attachment on a brand-new conversation's first message always reaches `/api/media/upload-url` while `chatId` is still null client-side, and its row is created with `chat_id: null`. Nothing ever backfilled it — every client-side status mechanism (Realtime, catch-up, the poll) filters by `chat_id`, so a still-null row could never be found again, permanently stuck showing "Processing…" even after the server finished (confirmed live via a direct DB query: 3 items uploaded on a new conversation's first message, all `status: 'ready'` within seconds, all `chat_id: null` and all stuck client-side). Fixed by having `useChatTurn.ts`'s `send()` include the pending attachment ids in the `POST /api/sessions` body when it creates a new session, and having that route backfill `chat_id` server-side in the same request (`backfillMediaChatId`, `services/media/index.ts`), scoped to the resolved member and only touching still-null rows. See `System Docs/Utilities/Chat UI.md`'s "Media-item delivery tracking" section and `System Docs/Known Gaps.md` for the full incident writeup.
