...
| Rename / Delete | no per-row actions | **UI shipped** in this package's `ConversationSidebar.*`; wire via `page-wiring.md` §5 + the `DELETE` route |
...
drawer/
  ...
  page-wiring.md                               ← exact page.tsx edits (select, persist, rename, delete)
  migrations/
    0001_prompt_conversations.sql              ← table + index + updated_at trigger
  services/prompt/
    conversations.ts                           ← list / get / create / update / delete (house Result pattern)
  app/api/admin/conversations/
    route.ts                                   ← GET (list) + POST (create)
    [id]/route.ts                              ← GET (hydrate) + PATCH (append/rename) + DELETE
  components/admin/prompt-builder/
    ConversationSidebar.tsx                    ← UPDATED: per-row Rename + Delete (drop-in over the branch file)
    ConversationSidebar.module.css             ← UPDATED: kebab / menu / confirm / inline-rename styles
```
...
5. **`New`** → clears chat **and** `activeConversationId` → next send starts a new
   row.
6. **Rename** → inline input in the row → `PATCH /…/:id { title }`.
7. **Delete** → inline confirm in the row → `DELETE /…/:id`; if it was the active
   thread, the page resets to a fresh draft.
...
- [ ] Kebab → **Rename** → inline input; Enter saves, Esc/blur cancels; title
      persists across reload.
- [ ] Kebab → **Delete** → inline confirm; Cancel aborts; Delete removes the row
      and (if active) resets to a fresh draft; stays gone after reload.
- [ ] Cross-owner isolation on delete → an owner cannot delete another's thread.
