...
//   Stage 1 · Compile  — the wiring POSTs to /api/admin/prompt/preview; this modal opens
...
// the server and renders the returned output so the reviewer sees the real thing (Correction 2).
...
// OPEN QUESTION (OQ-7): 72 is a design choice, not a constraint found in the codebase. If a
// column width or an existing short-text convention exists, that wins — and the server must
// validate the same number.
...
// Correction 2: NOT called from the wiring; the wiring uses the server response.
...
/**
 * Pre-fill for the summary field: "Update identity, guardrails and knowledge".
 * Derived from the distinct section headings of the changed blocks so the common case is
 * one keystroke (Tab) away from publishable. The author can overwrite it freely.
 */
export function suggestSummary(changed: ChangedBlock[]): string {
  if (!changed.length) return ''
  const types = [...new Set(changed.map((b) => (SECTION_HEADING[b.type] ?? b.type).toLowerCase()))]
  const head =
...
interface CompilePublishModalProps {
  opened: boolean
  onClose: () => void
  /** true while the server preview request is in flight — shows the loading state. */
  compiling: boolean
  /** compiled prompt returned by the server (empty while compiling). */
  text: string
  set: ModalSet
  /**
   * The version this publish will create — activeSet.version + 1. Shown in BOTH stages, and
   * computed before the compile POST (it used to be the version returned by the response).
   * OPEN QUESTION (OQ-3): unverified that the server always increments by exactly 1. If it
   * doesn't, drop the number from the stage-2 badge and the Publish button and let the success
   * toast name the real version from the response.
   */
  version: number
  /**
   * Token count of the currently live compiled prompt, for the stage-3 delta.
   * OPEN QUESTION (OQ-2): no such count exists in the Blocks page's data path today. Until one
   * does, leave this UNSET and suppress the delta — do not pass 0, which renders the full token
   * count as if it were all new.
   */
  previousTokens?: number
  /** Active blocks in the set — stage 3 derives its change list from these. */
  blocks?: ChangedBlock[]
  /**
   * activeSet.last_compiled_at — the cut-off for "changed since".
   * OPEN QUESTION (OQ-1): not currently selected by getPromptSets.ts and not on the Blocks
   * page's PromptSet interface; I have not verified the column exists on prompt_sets. When
   * null, changedSince() falls back to listing EVERY block passed in.
   */
            ...
            <Stack gap={8}>
              <Group justify="space-between" gap="sm" wrap="wrap">
                ...
                <Text size="xs" c="dimmed" style={{ fontFamily: 'var(--mantine-font-family-monospace)' }}>
                  {delta >= 0 ? '+' : ''}
                  {delta.toLocaleString()} tokens · {tokens.toLocaleString()} total
                </Text>
              </Group>
              {changed.length ? (
