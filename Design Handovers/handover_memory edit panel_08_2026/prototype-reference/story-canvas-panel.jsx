  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', height: 22, margin: '2px 0' }}
      ...
      <button onClick={() => setOpen((v) => !v)} aria-label="Add a block"
        style={{ flexShrink: 0, width: 22, height: 22, borderRadius: 99, border: '1px solid var(--hl-border-strong)', background: 'var(--hl-surface)', color: 'var(--hl-muted)', display: 'grid', placeItems: 'center', cursor: 'pointer', opacity: open ? 1 : 0.35, transition: 'opacity .15s' }}
        onMouseEnter={(e) => { e.currentTarget.style.opacity = 1; e.currentTarget.style.color = 'var(--hl-text)'; }}
        onMouseLeave={(e) => { if (!open) e.currentTarget.style.opacity = 0.35; e.currentTarget.style.color = 'var(--hl-muted)'; }}>
        <SIcon n="plus" s={13} />
      </button>
      <div style={{ flex: 1, height: 1, background: open ? 'var(--hl-border-strong)' : 'transparent', transition: 'background .15s' }} />
      {open && (
