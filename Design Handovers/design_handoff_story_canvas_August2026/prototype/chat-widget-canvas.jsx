...
(function () {
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, maxWidth: 520, border: '1px solid var(--hl-border-strong)', borderRadius: 16, background: 'var(--hl-surface)', boxShadow: '0 18px 44px -26px var(--hl-shadow)', overflow: 'hidden', animation: 'hl-modal-in .26s cubic-bezier(.22,1,.36,1)' }}>
          ...
          <div style={{ padding: '16px 18px 18px' }}>
            <h4 style={{ margin: 0, fontFamily: 'var(--font-display)', fontWeight: 500, fontSize: 23, lineHeight: 1.16, letterSpacing: '-.01em', color: 'var(--hl-text)', textWrap: 'pretty' }}>{message.title}</h4>
            ...
            {K.slots && (
              <React.Fragment>
                <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--hl-faint)' }}>— add them whenever you find them</span>
                </div>
  ...
  function Messages({ messages, loading, stories, keepDisabled, onKeep, onResend, onRegenerate, onVersion, onRate, onCopy, onFeedback, onEdit, onRetry, onMemSave, onMemRewrite, onMemDiscard, onMemOpen, onOffer }) {
  ...
  function StoryChat() {
    ...
    const [canvas, setCanvas] = useState({ open: false, storyId: null, memId: null });
    const [sideW, setSideW] = useState(() => { try { return Number(localStorage.getItem('hl.sideW')) || 264; } catch (e) { return 264; } });
    const [canvasW, setCanvasW] = useState(() => { try { return Number(localStorage.getItem('hl.canvasW')) || 440; } catch (e) { return 440; } });
    ...
    /* Below this the split stops being a split: the canvas takes the whole
       conversation area as a sheet rather than starving the transcript. */
    const canvasAsSheet = isMobile || bodyW < 980;
    useEffect(() => {
      if (canvasAsSheet) return;
      setCanvasW((c) => scClamp(c, 320, Math.max(320, bodyW - sideW - 360)));
    }, [bodyW, sideW, canvasAsSheet]);
    useEffect(() => { try { localStorage.setItem('hl.sideW', String(sideW)); localStorage.setItem('hl.canvasW', String(canvasW)); } catch (e) {} }, [sideW, canvasW]);
    ...
    const memSave = useCallback((id, storyId) => {
      ...
      if (target) {
        ...
        setMemories((p) => [...p, { id: 'k' + Date.now(), msgId: id, title: msg.title, passage: msg.passage,
    ...
    const createStory = (name, description) => { setStories((p) => [...p, { id: 'st' + Date.now(), name, tagline: description }]); setBeginOpen(false); flash('Story created'); };
    ...
    const openCanvas = (storyId, memId) => {
      setCanvas({ open: true, storyId: storyId || activeStoryId, memId: memId || null });
      setMobileNav(false);
      if (!isMobile) setFull(true);   // three columns need the room
    };
    const closeCanvas = () => setCanvas((c) => ({ ...c, open: false, memId: null }));
          ...
          <div ref={bodyRef} style={{ position: 'relative', display: 'flex', flex: 1, minHeight: 0 }}>
            ...
            {!isMobile && (
              <SCCurtain label="Resize the menu" onStart={() => sideW} onReset={() => setSideW(264)}
                onMove={(b, d) => setSideW(scClamp(b + d, 190, paneMax(canvas.open ? canvasW : 0)))} />
            )}
            {isMobile && mobileNav && (
              <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, height: '100%' }}>
              {hasStarted ? <Messages messages={messages} loading={loading} stories={stories} keepDisabled={!!streamingId || loading || (() => { const t = messages.filter((m) => m.role === 'tool'); const last = t[t.length - 1]; return !!last && (last.state === 'running' || last.state === 'draft'); })()} onKeep={() => runMemory(null, '')} onResend={resendMessage} onRegenerate={regenerate} onVersion={showVersion} onRate={rateMessage} onCopy={copyMessage} onEdit={editMessage} onRetry={retryMessage} onMemSave={memSave} onMemRewrite={memRewrite} onMemDiscard={memDiscard} onMemOpen={memOpen} onOffer={handleOffer} onFeedback={submitFeedback} /> : (
            ...
            {canvas.open && !canvasAsSheet && (
              <>
                <SCCurtain label="Resize the story canvas" onStart={() => canvasW} onReset={() => setCanvasW(440)}
                  onMove={(b, d) => setCanvasW(scClamp(b - d, 320, paneMax(sideW)))} />
                <aside style={{ width: canvasW, minWidth: canvasW, maxWidth: canvasW, flex: '0 0 auto', background: 'var(--hl-bg-2)' }}>{canvasEl}</aside>
              </>
            )}
            {canvas.open && canvasAsSheet && (
              <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, left: isMobile ? 0 : sideW + 9, zIndex: 22, background: 'var(--hl-bg-2)', borderLeft: '1px solid var(--hl-border)' }}>{canvasEl}</div>
            )}
