...
(() => {
  ...
  const stylesheet =
    ...
    '  justify-content:center;gap:6px;text-align:center;padding:12px;box-sizing:border-box;' +
    '  background:#fdf3ef;color:#9a3412;user-select:none;' +
    '  font:13px/1.45 system-ui,-apple-system,sans-serif}' +
    '.attr-error svg{opacity:.8}' +
    '.attr-error .cap{max-width:92%;font-weight:600;letter-spacing:.01em}' +
    '.attr-error .sub{font-size:11px;max-width:92%;opacity:.85}' +
    '.attr-error code{font:10px/1.3 ui-monospace,Menlo,monospace;background:rgba(154,52,18,.1);' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub"><u>Add a photo</u></div></div>' +
        '  <div class="attr-error" part="attribution-error">' + warnIcon +
        '    <div class="cap">Unsplash photo needs attribution</div>' +
        '    <div class="sub">Set <code>credit</code> and <code>credit-href</code> on this ' +
        '<code>&lt;image-slot&gt;</code> to show the photo.</div></div>' +
      ...
      this._empty.addEventListener('click', () => {
        // Open the host photo picker (stock library + built-in upload) when
        // editable; fall back to a local file browse where no host is present.
