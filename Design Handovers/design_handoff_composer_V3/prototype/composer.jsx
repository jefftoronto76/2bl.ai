...
(function () {
  ...
  /* Prompt-set picker for the composer topbar — choose which set new blocks
     save into, or spin up a new one inline. Reuses the Blocks header's ps-* CSS. */
  function PromptSetPicker({ sets, value, setValue, onCreate }) {
    ...
    useEffect(() => {
      if (!open) return;
      const f = (e) => { if (ref.current && !ref.current.contains(e.target)) { setOpen(false); setCreating(false); setName(''); } };
      document.addEventListener('mousedown', f); return () => document.removeEventListener('mousedown', f);
    }, [open]);
    const set = sets.find((s) => s.value === value) || sets[0];
    function create() { const nm = name.trim(); if (!nm) return; onCreate(nm); setName(''); setCreating(false); setOpen(false); }
    return (
      <div className="ps-select" ref={ref}>
        ...
        {open && (
          <div className="ps-menu" role="listbox">
            {sets.map((s) => (
              <button key={s.value} className={`ps-item ${s.value === value ? 'on' : ''}`} role="option" aria-selected={s.value === value} onClick={() => { setValue(s.value); setOpen(false); }}>
                ...
                <span className="ps-item-check">{s.value === value ? <I.Check size={14} /> : null}</span>
              </button>
            ))}
            <div className="ps-sep" />
            {creating ? (
              <div className="ps-create">
                <input className="ps-create-input" autoFocus value={name} placeholder="New prompt set name…"
                  onChange={(e) => setName(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') create(); if (e.key === 'Escape') { setCreating(false); setName(''); } }} />
                <button className="ps-create-go" disabled={!name.trim()} onClick={create}>Create</button>
              </div>
            ) : (
              <button className="ps-newitem" onClick={() => setCreating(true)}><I.Plus size={14} />New prompt set…</button>
            )}
          </div>
        )}
      </div>
  ...
  function Composer({ notify, userName }) {
    ...
    const [sets, setSets] = useState(() => window.PROMPT_SETS || [
    ...
    const [promptSet, setPromptSet] = useState(() => sets[0].value);
    ...
    function createSet(name) {
      ...
      const s = { value, label: name, version: 1, status: 'Draft' };
      setSets((arr) => [...arr, s]);
      setPromptSet(value);
      notify && notify({ title: 'Prompt set created', message: `“${name}” is ready — new blocks will save here.` });
    }

    function saveDraft(i) {
      setDrafts((d) => d.filter((_, idx) => idx !== i));
      notify && notify({ title: 'Block saved', message: `Added to ${activeSet.label}.` });
      ...
      <div className="composer-screen" data-screen-label="Admin · Composer">
        ...
        <div className="cmp-main">
          <div className="cmp-topbar">
            ...
            <div className="cmp-ps">
              <span className="cmp-ps-label">Building in</span>
