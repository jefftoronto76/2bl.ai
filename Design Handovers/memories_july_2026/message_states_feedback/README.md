...
> **Updated.** Two things changed after this package was first written:
> 1. The assistant action row now **leads with a `bookmark` action** (Keep this as a memory) —
>    see `design_handoff_memories`. It is the first item in the row, before Copy.
> 2. Action rows were **hover-only, which made them unreachable on touch**. They are now always
>    visible under `@media (hover: none)`, with 34×34 targets. See Mobile, below.
>
> Visitor (user) messages now carry their own Edit / Copy / Send again row —
> that is a separate package: `design_handoff_visitor_message_actions`.
...
**Behavior:**
- Thumbs sit in `MessageActions`, after the bookmark / Copy / Regenerate / version group,
  separated by a hairline divider, always rendered at ~50% opacity, full opacity on row hover — not hidden entirely, so the affordance is discoverable without cluttering every message.
...
The whole action row was hover-gated, which meant that on any touch device Copy, Regenerate,
the version arrows, the thumbs — and later the memory bookmark — were **invisible and
unreachable**. This is the single most important correction in this update.

```css
@media (hover: none) {
  .hl-acts { opacity: 1 !important; }
  .hl-acts button { width: 34px; height: 34px; }
}
```

- Rows are always visible on coarse pointers; buttons grow 28 → 34.
- 34px is still under the 44px guideline. Acceptable here because every action in the row is
  non-destructive and recoverable. If strict compliance is required, keep the bookmark, Copy and
  thumbs at 44px and move Regenerate + the version carousel into an overflow — **do not shrink
  the bookmark**, it is the row's most important action.
- The feedback popover is `max-width: 340` and indented 44px from the rail. At 375px it has
  ~40px of slack; verify the reason chips wrap rather than clip, and raise chip height to 44px
  on touch.
- The scroll-into-view fix on popover open matters more on mobile, where the keyboard can occupy
  half the viewport. Use the nearest scrollable ancestor — never `scrollIntoView`.
- Respect `prefers-reduced-motion` for shake / pop / blink; on mobile these read as jank more
  readily than on desktop.
...
- [ ] All new icons resolve: Check, AlertTriangle, Square (stop), RefreshCw, ChevronLeft/Right, ThumbsUp/Down, Copy, X, Bookmark.
- [ ] Touch: the full action row is visible without hover, targets ≥34px, nothing clipped at 375px.
- [ ] The bookmark is first in the row and never hidden behind hover or an overflow.
