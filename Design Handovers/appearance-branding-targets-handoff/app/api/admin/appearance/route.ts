...
// ─────────────────────────────────────────────────────────────────────────────
// CHANGES vs. main (see docs/appearance-branding-targets.md §3):
//   • `target` query param ('storefront' | 'admin', default 'storefront') —
//     selects which tenant_branding row to read/write. Rows are now keyed by
//     (tenant_id, target); the upsert conflict target becomes 'tenant_id,target'.
//   • GET_SELECT returns the two read-only sync columns: defaults_synced_at,
...
export async function GET(req: Request) {
  ...
  if (data === null) {
    ...
    // Defaults carry no sync history yet.
