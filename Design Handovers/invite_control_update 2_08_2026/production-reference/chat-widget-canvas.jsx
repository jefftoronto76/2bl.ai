...
(function () {
  ...
  /* ── Sidebar ──────────────────────────────────────────────────────────── */
  function Sidebar({ sessions, activeId, onNew, onSelect, onPrompt, onRowAction, onCreateStory, onInvite, onOpenStory, onOpenMediaPage, stories, memories, onClose, query, setQuery, width }) {
    const [convOpen, setConvOpen] = useState(true);
    const [menuId, setMenuId] = useState(null);
    const [storyMenuOpen, setStoryMenuOpen] = useState(false);
    const [selectedStoryId, setSelectedStoryId] = useState(stories[0] ? stories[0].id : null);
    useEffect(() => { if (!stories.find((s) => s.id === selectedStoryId)) setSelectedStoryId(stories[0] ? stories[0].id : null); }, [stories]);
    ...
    return (
      <aside style={{ width: width || 264, minWidth: width || 264, maxWidth: width || 264, flex: '0 0 auto', height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--hl-surface-2)', borderRight: 'none' }}>
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8, padding: '14px 10px 10px 16px', minHeight: 28, borderBottom: '1px solid var(--hl-border)' }}>
          <button onClick={() => setStoryMenuOpen((v) => !v)} disabled={!stories.length} style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1, minWidth: 0, border: 'none', background: 'transparent', padding: '4px 2px', cursor: stories.length ? 'pointer' : 'default', textAlign: 'left' }}>
            <span style={{ flex: 1, minWidth: 0, fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 17, color: 'var(--hl-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{selectedStory ? selectedStory.name : 'All stories'}</span>
            {stories.length > 0 && <Icon n="chevron" s={14} style={{ color: 'var(--hl-faint)', transform: storyMenuOpen ? 'rotate(180deg)' : 'none', transition: 'transform .15s', flexShrink: 0 }} />}
          </button>
          {onClose && <button aria-label="Close menu" onClick={onClose} style={iconBtn}><Icon n="x" s={18} /></button>}
          {storyMenuOpen && (
            <div style={{ position: 'absolute', left: 10, top: '100%', zIndex: 6, marginTop: 2, width: 232, background: 'var(--hl-surface)', border: '1px solid var(--hl-border)', borderRadius: 12, boxShadow: '0 18px 44px -18px var(--hl-shadow)', padding: 6 }}>
              {stories.map((st) => (
                <button key={st.id} onClick={() => { setSelectedStoryId(st.id); setStoryMenuOpen(false); onOpenStory && onOpenStory(st.id); }} style={{ ...menuItem, fontWeight: st.id === selectedStoryId ? 600 : 400, whiteSpace: 'nowrap' }} onMouseEnter={hov} onMouseLeave={unhov}>
                  <Icon n="bookOpen" s={14} style={{ color: 'var(--hl-muted)', flexShrink: 0 }} /><span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{st.name}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div style={{ padding: '10px 12px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {showSearch && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 10, border: '1px solid var(--hl-border)', background: 'var(--hl-surface)', marginTop: 4 }}>
              <Icon n="search" s={15} style={{ color: 'var(--hl-faint)' }} />
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search your story" style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-body)', fontSize: 13.5, color: 'var(--hl-text)' }} />
            </div>
          )}
        </div>
              ...
              <button key={k.id} onClick={() => selectMode ? toggle(k.id) : onOpen(k.id)} disabled={saved}
                ...
                <span style={{ flexShrink: 0, display: 'grid', placeItems: 'center', width: 38, height: 38, borderRadius: 99, background: 'var(--hl-accent-soft)', color: 'var(--hl-accent)' }}><Icon n={K.icon} s={16} /></span>                <span style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    ...
                    <span style={{ flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.05em', color: 'var(--hl-faint)' }}>{k.date}{story ? ' · ' + story.name : ''}</span>
                  </span>
  ...
  function MediaCard({ item, onRetry, onAddToMemory, onEdit, onDelete }) {
    ...
    return (
      <div style={{ flex: '1 1 220px', minWidth: 180, maxWidth: 320, border: '1px solid var(--hl-border)', borderRadius: 14, background: 'var(--hl-surface)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ position: 'relative', aspectRatio: '16 / 10', background: hasThumb ? 'var(--hl-surface-2)' : MEDIA_TILE_BG[item.type] || 'var(--hl-surface-2)', display: 'grid', placeItems: 'center' }}>
          {hasThumb ? <img src={item.photoSrc} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
          <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--hl-text)', lineHeight: 1.25, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.original_filename}</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--hl-faint)' }}>{prettySize(item.file_size_bytes)} · {date}</span>
          </div>
        ...
        <div onClick={(e) => e.stopPropagation()} role="dialog" aria-label="Invite collaborators" style={{ position: 'relative', width: 'min(462px, 100%)', maxHeight: '92%', display: 'flex', flexDirection: 'column', background: 'var(--hl-surface)', border: '1px solid var(--hl-border-strong)', borderRadius: 20, boxShadow: '0 40px 100px -24px var(--hl-shadow)', animation: 'hl-modal-in .26s cubic-bezier(.22,1,.36,1)', overflow: 'hidden' }}>
          ...
          {stories && stories.length > 0 && (
            <div style={{ marginTop: 16 }}>
              ...
              <select value={storyId || ''} onChange={(e) => onStoryChange && onStoryChange(e.target.value)}
