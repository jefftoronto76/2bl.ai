...
full page source and its dependencies so every element on the lander (not
just the nav) is available for reference/rebuild:

- `Heirloom Lander - Summer 2026 - Story Canvas.html` — full page source.
- `icons.jsx`, `image-slot.js` — lander-relevant script dependencies
  (icon set, image-slot placeholders). Non-lander scaffolding from the
  prototype (chat widget, tweaks panel) has been left out — out of scope.
...
- `image-mapping.md` — maps each `<image-slot>` id (hero constellation
  ...
  for wiring in real photos in place of placeholders.
