/* chat.jsx — Heirloom chat panel (sidebar + header + messages + composer).
   Exports ChatPanel to window. */

/* ===========================================================================
 * 🔖 KEBAB MENU IMPLEMENTATION — MAP FOR CLAUDE CODE
 * ---------------------------------------------------------------------------
 * The per-row "⋮" overflow menu (Star / Rename / Invite / Move / Remove /
 * Delete) is implemented across FOUR spots in this file. Search the tags:
 *
 *   [KEBAB-1] RecordMenu  — the popover component .............. ~line 150
 *   [KEBAB-2] RecordRow   — the row + kebab trigger + star mark . ~line 195
 *   [KEBAB-3] openRowMenu — anchor → position the popover ....... ~line 1205
 *   [KEBAB-4] onMenuAction + dismiss effect — controller logic .. ~line 1230
 *
 * Everything else in this file (GUIDE_TURNS, Composer, modals, StoriesView,
 * UploadsView, etc.) is NOT part of the kebab feature — ignore it.
 * Token values (--hl-*) → production Tailwind classes: see ../TOKEN_MAP.md
 * ========================================================================= */
(function () {
  ...
  const NAV = [
    ...
    { icon: 'share', label: 'Share Heirloom', action: 'share' },
  ];
  ...
  // ┌─ [KEBAB-1] ───────────────────────────────────────────────────────────┐
  // │ The menu POPOVER. Fixed 212px wide, glassy panel, 6 items + a divider  │
  // │ before the danger "Delete" row. Rendered by the panel root (see        │
  // │ [KEBAB-4]) in section-relative coords so it survives overflow:hidden.  │
  // │ Pure UI — calls onAction(key); performs no mutation itself.            │
  // └───────────────────────────────────────────────────────────────────────┘
  ...
  // ┌─ [KEBAB-2] ───────────────────────────────────────────────────────────┐
  // │ The ROW + its kebab TRIGGER. `kebab` is the 26×26 "⋮" button (opacity  │
  // │ 0 until hover/open — see mobile note in README). `starMark` shows the  │
  // │ star at rest and swaps out for the kebab on hover. Row opens the menu  │
  // │ via onMenu(anchorEl, item) on click OR onContextMenu (right-click).    │
  // │ Inline rename input lives here too (Enter/blur commit, Esc cancel).    │
  // └───────────────────────────────────────────────────────────────────────┘
  ...
  // ---- Sidebar -----------------------------------------------------------
  function Sidebar({ expanded, setExpanded, style, activeView, onNav, convoCount, activeRecent, setActiveRecent, overlay, onPrompt, items, renamingId, menuForId, onRowMenu, onCommitRename, onCancelRename, stories, onOpenStory, onCreateStory, onInviteAll }) {
  ...
  function Header({ onClose, showMenu, onMenu, onShare, fullScreen, onToggleFull }) {
    ...
    return (
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 12px', height: 52, borderBottom: '1px solid var(--hl-border)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <button style={iconBtn} aria-label="Share Heirloom" title="Share Heirloom" onClick={onShare} onMouseEnter={(e) => hov(e, true)} onMouseLeave={(e) => hov(e, false)}><Icon name="share" size={17} /></button>
  ...
  function Composer({ onSend, disabled, onSaveChat, saved }) {
          ...
          {recording ? (
            ...
            <React.Fragment>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 4px 2px 2px' }}>
                ...
                <div style={{ flex: 1 }} />
                <button onClick={() => { setRecording(true); }} aria-label="Record voice" title="Record voice" style={iconBtn} onMouseEnter={hover} onMouseLeave={unhover}>
  ...
  function ShareModal({ onClose }) {
    ...
    return (
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)', animation: 'hl-fade .2s ease' }}>
        <div onClick={(e) => e.stopPropagation()} role="dialog" aria-label="Share Heirloom" style={{
  ...
  // ---- Panel -------------------------------------------------------------
  function ChatPanel({ onClose, emptyPrompt, sidebarStyle, fullScreen, onToggleFull }) {
    const sectionRef = useRef(null);
    const [narrow, setNarrow] = useState(false);
    const [expanded, setExpanded] = useState(sidebarStyle !== 'rail');
    ...
    useEffect(() => { setExpanded(!narrow && sidebarStyle !== 'rail'); }, [sidebarStyle, narrow]);
    useEffect(() => { bottomRef.current && bottomRef.current.scrollIntoView({ behavior: 'smooth' }); }, [messages, loading]);

    const newChat = () => { setMessages([]); setLoading(false); turnRef.current = 0; setActiveRecent(null); setSavedChat(false); setView('home'); };
    ...
    // ── [KEBAB-3] open + POSITION the popover (right-align under the kebab;
    //    flip up if it would overflow the bottom; clamp to the section).
    ...
    // ── [KEBAB-4] CONTROLLER: maps each menu key → a state mutation. In the
    //    real app these become your store actions (the prototype mutates local
    //    `items` + shows a toast). The dismiss useEffect just below closes the
    //    menu on Escape / scroll / resize.
    const onMenuAction = (key) => {
      ...
      else if (key === 'delete') { setItems((arr) => arr.filter((x) => x.n !== it.n)); if (activeRecent === it.n) setActiveRecent(null); showToast('Story deleted'); }
    };
    ...
    return (
      <section ref={sectionRef} style={{ height: '100%', display: 'flex', background: 'var(--hl-bg)', overflow: 'hidden', position: 'relative' }}>
        <Sidebar expanded={expanded} setExpanded={setExpanded} style={sidebarStyle} activeView={view} onNav={onNav} convoCount={items.length} activeRecent={activeRecent} setActiveRecent={(n) => { setActiveRecent(n); setView('home'); }} overlay={narrow} onPrompt={(p) => { setView('home'); send(p); if (narrow) setExpanded(false); }} items={items} renamingId={renamingId} menuForId={menu ? menu.n : null} onRowMenu={openRowMenu} onCommitRename={commitRename} onCancelRename={() => setRenamingId(null)} stories={stories} onOpenStory={openStoryDetail} onCreateStory={() => setCreateStory(true)} onInviteAll={() => openInvite(null)} />
        {narrow && expanded && (
          <div onClick={() => setExpanded(false)} style={{ position: 'absolute', inset: 0, zIndex: 35, background: 'rgba(0,0,0,0.45)' }} />
        )}
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, height: '100%' }}>
          <Header onClose={onClose} showMenu={narrow} onMenu={() => setExpanded(true)} onShare={() => setShare(true)} fullScreen={fullScreen} onToggleFull={onToggleFull} />
            ...
            {view === 'stories' ? (
              ...
              <React.Fragment>
                ...
                <div style={{ paddingTop: 6 }}>
                  <Composer onSend={(t) => { setSavedChat(false); send(t); }} disabled={loading} onSaveChat={() => setSavedChat(true)} saved={savedChat} />
                </div>
              </React.Fragment>
            )}
          </div>
