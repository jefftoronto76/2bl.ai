...
(function () {
  ...
  /* What makes a memory rich, evaluated against the visitor's own words as
     it's written — surfaced as a checklist while writing, then as gentle
     suggestion chips on the draft for anything still thin. Heuristic, not a
     model call: keyword presence per trait, 'maybe' when there's enough text
     to have said something but the keywords didn't show up. */
  const MEMORY_TRAITS = [
    { key: 'who', label: 'Who was there', ask: 'Who else was there with you?', test: /\b(mom|dad|mother|father|grandma|grandpa|grandmother|grandfather|brother|sister|friend|wife|husband|son|daughter|uncle|aunt|cousin|we|with him|with her|with them)\b/ },
    { key: 'when', label: 'When this was', ask: 'About when did this happen?', test: /\b(19\d{2}|20\d{2}|years? ago|summer|winter|spring|fall|autumn|morning|night|birthday|christmas|weekend|when i was|childhood|growing up)\b/ },
    { key: 'where', label: 'Where it happened', ask: 'Where were you when this happened?', test: /\b(at the|in the|on the|house|home|kitchen|school|park|beach|city|town|backyard|porch|room)\b/ },
    { key: 'detail', label: 'A specific detail', ask: 'Is there a small detail \u2014 a sound, a smell, something someone said \u2014 that sticks with you?', test: /\b(smell|sound|taste|felt|looked|heard|saw|light|colou?r|cold|warm|smiled|laughed|cried|voice|hands)\b/ },
    { key: 'why', label: 'Why it matters', ask: 'Why does this moment stay with you?', test: /\b(because|meant|matters|special|important|love|miss|proud|grateful|never forget|still think)\b/ },
  ];
  function scoreMemoryTraits(seed) {
    const text = (seed || []).filter((m) => m.role === 'user').map((m) => m.content || '').join(' ').toLowerCase();
    return MEMORY_TRAITS.map((t) => ({ key: t.key, label: t.label, ask: t.ask, status: t.test.test(text) ? 'yes' : (text.length > 40 ? 'maybe' : 'no') }));
  }
  ...
  function UploadingCard({ K, steps, traits }) {
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, maxWidth: 520, border: '1px solid var(--hl-border-strong)', borderRadius: 16, background: 'var(--hl-surface)', boxShadow: '0 18px 44px -26px var(--hl-shadow)', overflow: 'hidden', animation: 'hl-modal-in .26s cubic-bezier(.22,1,.36,1)' }}>
          ...
          <div style={{ padding: '15px 18px 16px' }}>
            ...
            {traits && (
              <div style={{ marginTop: 14, paddingTop: 13, borderTop: '1px solid var(--hl-border)', display: 'flex', flexDirection: 'column', gap: 7 }}>
                {traits.map((t, ti) => {
                  const revealed = i >= Math.floor((ti / traits.length) * steps.length);
                  const color = t.status === 'yes' ? 'var(--hl-accent)' : t.status === 'maybe' ? 'var(--hl-muted)' : 'var(--hl-faint)';
                  return (
                    <div key={t.key} style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: revealed ? 1 : 0.32, transition: 'opacity .35s' }}>
                      <span style={{ display: 'grid', placeItems: 'center', width: 15, height: 15, borderRadius: 99, flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, background: revealed ? (t.status === 'yes' ? 'var(--hl-accent-soft)' : 'var(--hl-surface-2)') : 'var(--hl-surface-2)', color: revealed ? color : 'var(--hl-faint)' }}>
                        {!revealed ? <span style={{ width: 4, height: 4, borderRadius: 99, background: 'currentColor' }} /> : t.status === 'yes' ? <Icon n="check" s={9} /> : t.status === 'maybe' ? '?' : <Icon n="x" s={9} />}
                      </span>
                      <span style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: revealed ? 'var(--hl-muted)' : 'var(--hl-faint)' }}>{t.label}</span>
                    </div>
                  );
                })}
              </div>
  ...
  function MemoryCard({ message, stories, onSave, onRewrite, onDiscard, onOpen, onExtra, onRetry, onOpenMedia, onSuggest }) {
    ...
    if (st === 'running') {
      return <UploadingCard K={K} steps={UPLOAD_TICKER[message.kindKey] || UPLOAD_TICKER.conversation} traits={message.traits} />;
    }
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, maxWidth: 520, border: '1px solid var(--hl-border-strong)', borderRadius: 16, background: 'var(--hl-surface)', boxShadow: '0 18px 44px -26px var(--hl-shadow)', overflow: 'hidden', animation: 'hl-modal-in .26s cubic-bezier(.22,1,.36,1)' }}>
          ...
          <div style={{ padding: '16px 18px 18px' }}>
            ...
            {message.traits && message.traits.some((t) => t.status !== 'yes') && (
              <React.Fragment>
                <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={label}>Make it richer</span>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                  {message.traits.filter((t) => t.status !== 'yes').map((t) => (
                    <button key={t.key} onClick={() => onSuggest(t)} style={{ ...ghost, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 11px', fontSize: 12, borderStyle: 'dashed' }}
  ...
  function Messages({ messages, loading, stories, keepDisabled, onKeep, onResend, onRegenerate, onVersion, onRate, onCopy, onFeedback, onEdit, onRetry, onMemRetry, onMemSave, onMemRewrite, onMemDiscard, onMemOpen, onMemRetitle, onMemSuggest, onOffer, onRewriteOption, onStoryPrompt, onOpenMedia, onRetryUpload, onBookmarkUpload, onAddUploadToMemory }) {
          ...
          {messages.map((m) => (
            m.role === 'tool' ? (
              <MemoryCard key={m.id} message={m} stories={stories} onOpenMedia={onOpenMedia}
                ...
                onSuggest={(trait) => onMemSuggest(m.id, trait)} />
            ) : m.upload ? (
  ...
  function StoryChat() {
    ...
    const [coverBackEditing, setCoverBackEditing] = useState(null); // null | { storyId, kind: 'cover'|'back' }
    const [previewStoryId, setPreviewStoryId] = useState(null); // null | storyId
    ...
    const runMemory = useCallback((existingId, note, prior, kindKey, photoSrc) => {
      ...
      if (id) {
        ...
        id = 'mem' + Date.now();
        const traits = (!kindKey || kindKey === 'conversation') ? scoreMemoryTraits(seed) : null;
        setMessages((m) => [...m, { id, role: 'tool', kind: 'memory', kindKey: kindKey || 'conversation', state: 'running', photoSrc, traits }]);
      }
    ...
    /* A suggestion chip on the draft — same rewrite pipeline as the generic
       Rewrite button, just seeded with the guide asking the one specific
       question that trait was missing, instead of the open-ended prompt. */
    const memSuggest = useCallback((id, trait) => {
      const prior = messagesRef.current.find((x) => x.id === id);
      if (!prior) return;
      rewritePendingRef.current = { title: prior.title, passage: prior.passage, kindKey: prior.kindKey };
      const aid = 'a' + Date.now();
      const ask = trait.ask;
      setMessages((m) => [...m.filter((x) => x.id !== id), { id: aid, role: 'assistant', content: '', full: ask, turn: 0, streaming: true, stopped: false, versions: [ask], versionIdx: 0, rating: null }]);
      streamText(aid, ask);
    }, [streamText]);
    ...
    const sessionMediaItems = SEED_MEDIA_ITEMS.filter((it) => it.sessionId === (activeIdRef.current || 'live'));
    ...
    useEffect(() => {
      if (panelOpen && !prevPanelOpenRef.current) {
        ...
        setCardW(scClamp(Math.round(total * 0.55), 320, Math.max(320, total - sideW - 9 - 380)));
      }
    ...
    const canvasEl = canvas.menu
      ...
      : <Deck memories={deck} story={canvasStory ? canvasStory.name : 'Your story'} storyId={canvas.storyId}
          cover={canvasStory && canvasStory.cover} backPage={canvasStory && canvasStory.backPage}
          onOpen={(id) => setCanvas((c) => ({ ...c, memId: id }))} onReorder={reorderDeck}
          onAdd={(kind) => { if (kind === 'memory') setAddMemToStory(canvas.storyId); else setCoverBackEditing({ storyId: canvas.storyId, kind }); }}
          onEditEnd={(kind) => setCoverBackEditing({ storyId: canvas.storyId, kind })}
          onRemoveEnd={(kind) => { setStories((prev) => prev.map((s) => s.id === canvas.storyId ? { ...s, [kind === 'cover' ? 'cover' : 'backPage']: undefined } : s)); flash((kind === 'cover' ? 'Cover' : 'Back page') + ' removed'); }}
          onPreview={() => { if (isMobile) { flash('Preview looks best on a bigger screen \u2014 open Heirloom on your computer to see the finished book.'); } else { setPreviewStoryId(canvas.storyId); } }}
    ...
    const sidebarEl = (
      <Sidebar width={isMobile ? 264 : sideW} sessions={sessions} activeId={activeId} onNew={newChat} memories={memories} onOpenMediaPage={() => setMediaPageOpen(true)}
    ...
    return (
      <>
        ...
        <div role="dialog" aria-modal="true" aria-label="Start your story" inert={!open ? '' : undefined}
          ...
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 10px', height: 52, borderBottom: '1px solid var(--hl-border)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <button onClick={openMediaGallery} aria-label="Media from this chat" title="Media from this chat"
                style={{ position: 'relative', display: 'grid', placeItems: 'center', width: 34, height: 34, borderRadius: 9, border: 'none', background: 'transparent', color: 'var(--hl-muted)', cursor: 'pointer', marginRight: 4 }}
                onMouseEnter={(e) => (e.currentTarget.style.background = 'color-mix(in srgb, var(--hl-text) 6%, transparent)')}
                onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
                <Icon n="image" s={16} />
                {sessionMediaItems.length > 0 && (
                  <span style={{ position: 'absolute', top: 2, right: 2, minWidth: 14, height: 14, padding: '0 3px', borderRadius: 7, background: 'var(--hl-accent)', color: 'var(--hl-on-accent)', fontFamily: 'var(--font-mono)', fontSize: 9, display: 'grid', placeItems: 'center' }}>{sessionMediaItems.length}</span>
                )}
              </button>
          ...
          <div ref={bodyRef} style={{ position: 'relative', display: 'flex', flex: 1, minHeight: 0 }}>
            {!isMobile && (
              <div style={{ width: sideW, flexShrink: 0, overflow: 'hidden', transition: 'width .4s cubic-bezier(.22,1,.36,1)' }}>
                <div style={{ width: isMobile ? 264 : sideW, height: '100%' }}>{sidebarEl}</div>
              </div>
            )}
            {!isMobile && !panelOpen && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: '1 1 0%', minWidth: 380, height: '100%' }}>
              {hasStarted ? <Messages messages={messages} loading={loading} stories={stories} keepDisabled={!!streamingId || loading || (() => { const t = messages.filter((m) => m.role === 'tool'); const last = t[t.length - 1]; return !!last && (last.state === 'running'); })()} onKeep={() => runMemory(null, '')} onResend={resendMessage} onRegenerate={regenerate} onVersion={showVersion} onRate={rateMessage} onCopy={copyMessage} onEdit={editMessage} onRetry={retryMessage} onMemRetry={memRetry} onMemSave={memSave} onMemRewrite={memRewrite} onMemDiscard={memDiscard} onMemOpen={memOpen} onMemRetitle={memRetitle} onMemSuggest={memSuggest} onOffer={handleOffer} onRewriteOption={handleRewriteOption} onStoryPrompt={handleStoryPrompt} onFeedback={submitFeedback} onOpenMedia={(kind, title, src) => setLightbox({ kind, title, src })} onRetryUpload={retryUpload} onBookmarkUpload={bookmarkUpload} onAddUploadToMemory={onAddUploadToMemory} /> : (
            ...
            {!isMobile && panelOpen && (
              <SCCurtain label="Resize the memory panel" onStart={() => cardW}
                onReset={() => setCardW(scClamp(Math.round(window.innerWidth * 0.55), 320, Math.max(320, window.innerWidth - sideW - 9 - 380)))}
                onMove={(b, d) => setCardW(scClamp(b - d, 320, window.innerWidth - sideW - 9 - 380))} />
            )}
            {!isMobile && (
        ...
        <div role="dialog" aria-modal="true" aria-label="Your story" inert={!canvas.open ? '' : undefined}
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
            ...
            <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', justifyContent: 'center' }}>
              <div key={canvas.menu ? 'menu' : deckIndex >= 0 ? 'card-' + deck[deckIndex].id : 'deck'} className={deckIndex >= 0 ? 'sc-slide-right' : 'sc-slide-up'} style={{ width: '100%', maxWidth: 780, height: '100%', minHeight: 0 }}>{canvasEl}</div>
            </div>
          </div>
        </div>
        ...
        {coverBackEditing && (
          <CoverBackPanel kind={coverBackEditing.kind}
            initial={(stories.find((s) => s.id === coverBackEditing.storyId) || {})[coverBackEditing.kind === 'cover' ? 'cover' : 'backPage']}
            onClose={() => setCoverBackEditing(null)}
            onSave={(payload) => {
              setStories((prev) => prev.map((s) => s.id === coverBackEditing.storyId ? { ...s, [coverBackEditing.kind === 'cover' ? 'cover' : 'backPage']: payload } : s));
              flash((coverBackEditing.kind === 'cover' ? 'Cover' : 'Back page') + ' saved');
              setCoverBackEditing(null);
            }}
            onRemove={() => {
              setStories((prev) => prev.map((s) => s.id === coverBackEditing.storyId ? { ...s, [coverBackEditing.kind === 'cover' ? 'cover' : 'backPage']: undefined } : s));
              flash((coverBackEditing.kind === 'cover' ? 'Cover' : 'Back page') + ' removed');
              setCoverBackEditing(null);
            }} />
        )}
        {previewStoryId && (
          <PreviewModal story={(stories.find((s) => s.id === previewStoryId) || {}).name || 'Your story'}
            cover={(stories.find((s) => s.id === previewStoryId) || {}).cover}
            backPage={(stories.find((s) => s.id === previewStoryId) || {}).backPage}
            memories={memories.filter((k) => k.storyId === previewStoryId).map((k) => ({ ...k, kind: k.kind || 'conversation', passage: k.passage || '' }))}
  ...
  style.textContent = `
    ...
    @keyframes sc-slide-right-in { from { opacity: 0; transform: translateX(36px); } to { opacity: 1; transform: translateX(0); } }
    .sc-slide-up { animation: sc-rise .34s cubic-bezier(.22,1,.36,1); }
    .sc-slide-right { animation: sc-slide-right-in .34s cubic-bezier(.22,1,.36,1); }
    .mm-zoom-frame:hover .mm-zoom-hint, .mm-zoom-frame:focus-visible .mm-zoom-hint { opacity: 1 !important; }
    @media (prefers-reduced-motion: reduce) { .sc-slide-up, .sc-slide-right { animation: none !important; } }
