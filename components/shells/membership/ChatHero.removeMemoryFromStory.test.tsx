...
async function openPanelAndPicker() {
  ...
  // Targeted by StoryPicker's own stable testid, not title/label text — see
  // ChatHero.assignMemoryToStory.test.tsx's own openPanelAndPicker for why.
  fireEvent.click(screen.getByTestId('story-picker-trigger'));
  await waitFor(() => expect(screen.getByRole('button', { name: 'Remove from "Summer at the Lake"' })).toBeInTheDocument());
}
...
describe('ChatHero — remove memory from story', () => {
  ...
  it('after removal, the trigger reverts to the accent Plus circle', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Add to a story' })).toBeInTheDocument());
  });
});
