...
// Who invited each member (by user id). '—' / absent = seeded/platform-created, shown as a dash.
// Platform-seeded owners/admins trace to the SBL founder; others to their tenant owner/admin.
const INVITED_BY = {
  u01: 'Jeff Lougheed', u02: 'Dana Whitfield', u03: 'Jeff Lougheed', u04: 'Priya Nair',
  u05: 'Jeff Lougheed', u06: 'Sarah Chen',     u07: 'Jeff Lougheed', u08: 'Jeff Lougheed',
  u09: 'Dana Whitfield', u10: 'Jeff Lougheed', u11: 'Sarah Chen',    u12: 'Priya Nair',
  u13: 'Elena Rossi',   u14: 'Dana Whitfield',
};

/* ── Link tracking ───────────────────────────────────────────────────────
   Every link we send — a member invite, a shared report, a shared chat —
   moves through one lifecycle: sent → delivered → opened → accepted.
   `reached` is the furthest stage a recipient got to; the per-stage keys
   carry a human timestamp for each stage actually reached (absent beyond
   it). This is what lets the admin answer "did they click the link?".
   ─────────────────────────────────────────────────────────────────────── */
const LINK_STAGES = ['sent', 'delivered', 'opened', 'accepted'];
const LINK_STAGE_META = {
  sent:      { label: 'Sent',      color: '#a99e8b' },
  delivered: { label: 'Delivered', color: '#1c7ed6' },
  opened:    { label: 'Opened',    color: '#C8542E' },
  accepted:  { label: 'Accepted',  color: '#2d6a4f' },
};
const stageIndex = (s) => LINK_STAGES.indexOf(s);
// What the terminal ('accepted') stage means in words, per link type.
const LINK_TYPE_META = {
  invite: { label: 'Invite',        terminal: 'Accepted' },
  report: { label: 'Shared report', terminal: 'Viewed'   },
  chat:   { label: 'Shared chat',   terminal: 'Opened'    },
};

// Invite-link progress per invited member (keyed by user id).
const INVITE_LINK = {
  u04: { reached: 'opened',    sent: '5d ago', delivered: '5d ago', opened: '2d ago' },
  u08: { reached: 'delivered', sent: '9d ago', delivered: '9d ago' },
};

// Every link we've sent (any type) — powers the dedicated Link Activity view.
const SENT_LINKS = [
  { id: 'lk1', type: 'invite', label: 'Team invite',            to: 'Tom Alvarez',    email: 'tom@riverside.health',   reached: 'opened',    at: 'Opened 2d ago' },
  { id: 'lk2', type: 'invite', label: 'Team invite',            to: 'Jordan Park',    email: 'jordan@hugs.community',  reached: 'delivered', at: 'Delivered 9d ago' },
  { id: 'lk3', type: 'report', label: 'Q2 usage report',        to: 'Sarah Chen',     email: 'sarah.chen@summit.re',   reached: 'accepted',  at: 'Viewed 1d ago' },
  { id: 'lk4', type: 'chat',   label: 'Onboarding walkthrough', to: 'Dana Whitfield', email: 'dana@acme.com',          reached: 'opened',    at: 'Opened 4h ago' },
  { id: 'lk5', type: 'report', label: 'March invoice (PDF)',    to: 'Priya Nair',     email: 'priya@riverside.health', reached: 'sent',      at: 'Sent 26m ago' },
  { id: 'lk6', type: 'chat',   label: 'Shared thread: pricing', to: 'Henry Osei',     email: 'henry@ledger.fi',        reached: 'accepted',  at: 'Opened 3d ago' },
  { id: 'lk7', type: 'invite', label: 'Admin invite',           to: 'Caleb Wright',   email: 'caleb@riverside.health', reached: 'accepted',  at: 'Accepted 20d ago' },
  { id: 'lk8', type: 'report', label: 'Weekly digest',          to: 'Elena Rossi',    email: 'elena@heirloom.app',     reached: 'delivered', at: 'Delivered 6h ago' },
];
...
/* ── Message feedback (message_feedback table) ──────────────────────────
   One row per rated assistant message: session_id + message_index locate the
   message inside SESSION_MESSAGES[session_id]; rating/tags/detail are the
   visitor's thumbs + reason chips + optional note, verbatim from the CD
   feedback-popover handoff. ─────────────────────────────────────────────── */
const MESSAGE_FEEDBACK = [
  { id: 'mf1', session_id: 's01', message_index: 3, rating: 'down', tags: ['Off tone'], detail: 'Jumped straight to selling the sprint before understanding our situation.', created_at: '2026-06-18T14:29:00' },
  { id: 'mf2', session_id: 's04', message_index: 3, rating: 'up', tags: ['Great question'], detail: 'Actually made me think differently about this.', created_at: '2026-06-17T18:15:00' },
  { id: 'mf3', session_id: 's07', message_index: 1, rating: 'down', tags: ['Repetitive'], detail: null, created_at: '2026-06-16T16:39:00' },
  { id: 'mf4', session_id: 's09', message_index: 1, rating: 'down', tags: ['Too generic'], detail: 'Wanted an actual range, even rough.', created_at: '2026-06-15T15:48:00' },
  { id: 'mf5', session_id: 's09', message_index: 3, rating: 'up', tags: ['Felt personal'], detail: null, created_at: '2026-06-15T15:51:00' },
];
function feedbackFor(sessionId, messageIndex) {
  return MESSAGE_FEEDBACK.find((f) => f.session_id === sessionId && f.message_index === messageIndex) || null;
}
function feedbackCounts(sessionId) {
  const rows = MESSAGE_FEEDBACK.filter((f) => f.session_id === sessionId);
  return { up: rows.filter((f) => f.rating === 'up').length, down: rows.filter((f) => f.rating === 'down').length };
}

/* ── Session transcripts (chat_messages, representative excerpts) ────────
   Not the full logged history for every session (some `msgs` counts run
   much higher) — enough of each conversation to exercise delivery states,
   stop/regenerate + version history, and rated replies. role: 'user' |
   'assistant'. User messages carry `status` (sending/sent/failed). Assistant
   messages may carry `stopped` + `versions`/`versionIdx` for regenerated
   replies. ─────────────────────────────────────────────────────────────── */
const SESSION_MESSAGES = {
  s01: [
    { role: 'user', content: 'Hi, I saw your post about scaling a two-person agency into something bigger. That is basically our story right now.', status: 'sent' },
    { role: 'assistant', content: "That's a great place to be, and a hard one. What's the actual bottleneck right now: new business, or delivery capacity once you land the work?", ttft_ms: 420 },
    { role: 'user', content: 'Delivery, mostly. We keep saying yes and then scrambling.', status: 'sent' },
    { role: 'assistant', content: 'That pattern usually means the founders are still doing senior work that could be systematized. Want me to walk through what a discovery sprint maps out?', ttft_ms: 610 },
    { role: 'user', content: "Maybe. What does it actually cost?", status: 'sent' },
    { role: 'assistant', content: "Pricing depends on scope, so I'd rather get that right on a call than guess here — can I point you to Jeff's booking link?", ttft_ms: 380 },
  ],
  s02: [
    { role: 'user', content: 'Do you work with resource extraction companies specifically, or just adjacent industries?', status: 'sent' },
    { role: 'assistant', content: "Mostly adjacent — logistics, equipment financing, land services. We stay out of direct extraction work itself. What's your business?", ttft_ms: 350 },
    { role: 'user', content: 'We do equipment leasing for mid-size mining operators.', status: 'failed' },
    { role: 'user', content: 'We do equipment leasing for mid-size mining operators.', status: 'sent' },
    { role: 'assistant', content: "Equipment leasing is squarely in our lane then. What's prompting you to look at outside advisory now?", ttft_ms: 730 },
  ],
  s03: [
    { role: 'user', content: "What's the difference between what you do and a regular business consultant?", status: 'sent' },
    { role: 'assistant', content: 'Most consultants sell a framework. Jeff works with a handful of clients at a time and stays in the room for execution, not just the plan.', ttft_ms: 310 },
    { role: 'user', content: 'Got it, thanks.', status: 'sent' },
  ],
  s04: [
    { role: 'user', content: "We're a small brokerage and I think we're underpricing our services compared to competitors.", status: 'sent' },
    { role: 'assistant', content: "That's a common blind spot — pricing tends to lag confidence, not the market. Have you looked at what's actually driving your close rate at the current price?", ttft_ms: 480 },
    { role: 'user', content: "Not really, we just match what everyone else charges.", status: 'sent' },
    { role: 'assistant', content: 'That\u2019s worth unpacking on a call. A lot of brokerages find they can move price without losing deals once they separate pricing from matching the pack.', ttft_ms: 560 },
    { role: 'user', content: "That's helpful, I'll book some time.", status: 'sent' },
  ],
  s05: [
    { role: 'user', content: 'How long have you been doing this?', status: 'sent' },
    { role: 'assistant', content: "Jeff's been advising founders and operators for about a decade, the last few years focused specifically on resource-adjacent industries.", ttft_ms: 340 },
    { role: 'user', content: 'Cool.', status: 'sent' },
  ],
  s06: [
    { role: 'user', content: 'hey', status: 'sent' },
    { role: 'assistant', content: "Hi! I'm Sage, here to help you think through what Natural Resource could do for you. What's on your mind?", ttft_ms: 300 },
  ],
  s07: [
    { role: 'user', content: "I run a small firm and I'm skeptical AI advisors can actually understand nuance.", status: 'sent' },
    { role: 'assistant', content: "Fair skepticism. I'm not here to replace the advice — I'm here to get you to Jeff faster with the right context already gathered.", ttft_ms: 610 },
    { role: 'user', content: "That's a very rehearsed answer.", status: 'sent' },
    { role: 'assistant', content: 'Noted — what would actually be useful to talk through right now?', ttft_ms: 390 },
  ],
  s08: [
    { role: 'user', content: 'What industries do you cover?', status: 'sent' },
    { role: 'assistant', content: 'Mostly resource-adjacent — logistics, land services, equipment finance, and similar operators looking to grow deliberately.', ttft_ms: 420 },
    { role: 'user', content: "We're in cold storage logistics.", status: 'sent' },
    { role: 'assistant', content: "That fits well. What's the growth question you're sitting with right now?", ttft_ms: 500 },
  ],
  s09: [
    { role: 'user', content: 'Can you tell me about the discovery sprint pricing?', status: 'sent' },
    { role: 'assistant', content: "It's a paid two-week engagement, priced on a call once we understand your situation — I can't quote a number here, but I can get you booked with Jeff.", ttft_ms: 700 },
    { role: 'user', content: 'Okay, and what happens after the sprint?', status: 'sent' },
    { role: 'assistant', content: 'You get a written map of where capacity is actually going, and a clear yes/no on whether ongoing advisory makes sense — no obligation either way.', ttft_ms: 450 },
    { role: 'user', content: "That's fair, thanks.", status: 'sent' },
  ],
  s10: [
    { role: 'user', content: 'What makes Natural Resource different from a typical advisory firm?', status: 'sent' },
    { role: 'assistant', content: 'Natural Resource works with a small number of clients at a time, so Jeff stays close to execution rather than handing off a deck and disappearing.', stopped: true, versions: ['Natural Resource is a boutique advisory prac—', 'Natural Resource works with a small number of clients at a time, so Jeff stays close to execution rather than handing off a deck and disappearing.'], versionIdx: 1, ttft_ms: 890 },
    { role: 'user', content: "That's helpful, keep going.", status: 'sent' },
    { role: 'assistant', content: "Happy to. What's the specific decision you're weighing right now — capital, a partnership, or something about growth?", ttft_ms: 410 },
  ],
};

/* ── Time to first token (chat_sessions.ttft_ms) ─────────────────────────
   Per-turn latency, measured send → first streamed token. Lives on each
   assistant message here; helpers roll it up per session and per day for
   the dashboard trend. Perf principle: first token should land under 1s. */
const TTFT_FLAG_MS = 1000;
function sessionTtftValues(sessionId) {
  return (SESSION_MESSAGES[sessionId] || []).filter((m) => m.role === 'assistant' && m.ttft_ms != null).map((m) => m.ttft_ms);
}
function avgTtft(sessionId) {
  const vals = sessionTtftValues(sessionId);
  return vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : null;
}
function overallAvgTtft() {
  const vals = INBOUND_CHATS.flatMap((s) => sessionTtftValues(s.id));
  return vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : null;
}
// Daily buckets derived from each session's updated_at (mock stand-in for a
// real per-message timestamp) — powers the dashboard sparkline.
function ttftTrend() {
  const byDay = {};
  for (const s of INBOUND_CHATS) {
    const day = s.updated_at.slice(0, 10);
    const vals = sessionTtftValues(s.id);
    if (!vals.length) continue;
    (byDay[day] || (byDay[day] = [])).push(...vals);
  }
  return Object.keys(byDay).sort().map((day) => ({ day, avg: Math.round(byDay[day].reduce((a, b) => a + b, 0) / byDay[day].length) }));
}
