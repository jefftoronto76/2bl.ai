// app/admin/prompt-studio/blocks/BlocksOverview.tsx
//
// Header summary for the Blocks page: a details grid (left) + the
// token-distribution donut (right). Replaces the bare SegmentedTokenMeter at
// the top of BlocksTable with a richer at-a-glance card.
//
// Everything except `version` / `status` is derived from the blocks already in
// client state, so the card updates live as blocks are toggled, edited, or
// deleted. `version` / `status` are optional — wire them from the prompt-studio
// publish state when available (see the handover doc §3).
...
export interface BlocksOverviewProps {
  /** All non-deleted blocks (active + disabled). Active blocks feed the donut. */
  blocks: BlockRow[];
  /** Published prompt version (e.g. 7) — optional; renders "—" when absent. */
  version?: number | null;
  /** Publish status label, e.g. "Live" / "Draft" — optional. */
  status?: string | null;
}
    ...
    <Card withBorder radius="md" p="md">
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="lg" style={{ alignItems: 'center' }}>
        ...
        <Stack gap="sm">
          ...
          {/* Legend — same colors as the donut + the per-row type badges */}
        ...
        <Stack gap={6} align="center">
          <TokenDonut blocks={donutBlocks} limit={TOKEN_LIMIT} />
