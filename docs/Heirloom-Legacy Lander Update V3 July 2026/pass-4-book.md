...
## The image
- Ship `book-keepsake.png` (in this bundle) to `public/heirloom/` and render with `next/image`. It is a **placeholder-grade AI render** — fine for launch, but treat it as swappable for real product photography later (same slot, no layout change).
