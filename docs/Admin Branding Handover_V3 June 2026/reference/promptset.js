...
export function PromptSetModal({ mode, set, onClose, onSave }) {
  ...
  const save = () => {
    ...
    onSave({ ...(set || {}), label: label.trim(), usage_type: usage, status, description: description.trim() })
  }
    ...
    <${M.Modal} opened=${true} onClose=${onClose} size="md" radius="md" centered
      ...
      <${M.Stack} gap="md">
        <${M.TextInput} label="Name" placeholder="Sage Base" required value=${label}
          error=${err.label} onChange=${(e) => setLabel(e.currentTarget.value)} data-autofocus />
      ...
      <${M.Stack} gap="xs">
        <${M.Group} justify="space-between" align="flex-start" gap="sm" wrap="nowrap">
          <${M.Group} gap="sm" align="center" wrap="wrap" style=${{ minWidth: 0 }}>
            ...
            ${set.is_master && html`<${M.Badge} size="sm" radius="sm" variant="filled" color="green">Master<//>`}
        ...
        <${M.Card} withBorder radius="sm" p="sm" style=${{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
          <${M.Stack} gap=${6}>
            ...
            ${set.is_master && html`<${MetaRow} label="Master">${html`<${M.Badge} size="sm" radius="sm" variant="filled" color="green">Master<//>`}<//>`}
...
export function PromptSetList({ scope, notify }) {
  ...
  const save = (s) => {
    setSets((prev) => {
      ...
      const created = { id: `new-${Date.now()}`, tenant_id: platform ? 'sbl' : scope, is_master: false, version: 1,
      ...
      ${groups.length === 0
        ...
        : groups.map(({ tenant, sets }) => html`
          <${M.Stack} key=${tenant.id} gap="xs">
            ...
            ${sets.map((s) => html`<${PromptSetCard} key=${s.id} set=${s} showTenant=${platform}
              onOpenInComposer=${() => openInComposer(s)} onView=${() => setViewing(s)}
              onEdit=${() => setModal({ mode: 'edit', set: s })} onDelete=${() => setConfirm(s)} />`)}
          <//>`)}
      ${modal && html`<${PromptSetModal} mode=${modal.mode} set=${modal.set} onClose=${() => setModal(null)} onSave=${save} />`}
