...
// ── Compile — assemble active blocks into the raw prompt (mirrors the compile pipeline:
// grouped by type in compile order, ordered within a type, plain markdown headings) ──
const SECTION_HEADING = { guardrail: 'Guardrails', identity: 'Identity', process: 'Process', knowledge: 'Knowledge', escalation: 'Escalation' }
function compilePrompt(activeBlocks) {
  const parts = []
  for (const type of D.ORDERED_BLOCK_TYPES) {
    const group = activeBlocks.filter((b) => b.type === type).sort((a, b) => (a.order || 99) - (b.order || 99))
...
function Overview({ blocks, set, onCompile, notify, showGuardrail }) {
      ...
      <${M.Card} withBorder radius="md" p="lg" style=${{ background: 'transparent' }}>
        <${M.Stack} gap="md" justify="space-between" style=${{ height: '100%' }}>
          ...
          <${M.Group} gap="sm">
            ...
            <${M.Button} onClick=${onCompile}>Compile & Publish<//>
          <//>
        <//>
      <//>
      <${M.Card} withBorder radius="md" p="lg" style=${{ background: 'transparent' }}>
...
/* ── Two-stage Compile → review → Publish modal ── */
function CompilePublishModal({ opened, onClose, compiling, text, set, version, onPublish, notify }) {
  const [copied, setCopied] = useState(false)
  useEffect(() => { if (!opened) setCopied(false) }, [opened])
  const tokens = D.tokensFor(text)
  ...
  const title = html`
    <${M.Group} gap="sm" align="center" wrap="nowrap">
      <${M.ThemeIcon} variant="light" color="brand" size=${34} radius="md"><${Ic.IconFileText} size=${18} /><//>
      <div>
        <${M.Text} fw=${600} size="sm" style=${{ lineHeight: 1.2 }}>Review compiled prompt<//>
        <${M.Text} c="dimmed" size="xs">${set.label}<//>
      </div>
    <//>`
  return html`
    <${M.Modal} opened=${opened} onClose=${onClose} centered size="xl" radius="md" title=${title}
      overlayProps=${{ backgroundOpacity: 0.55, blur: 2 }}
      styles=${{ title: { width: '100%' }, body: { paddingTop: 4 } }}>
      ${compiling
        ? html`
          <${M.Stack} align="center" justify="center" gap="md" py=${64}>
            <${M.Loader} color="brand" />
            <${M.Text} c="dimmed" size="sm">Compiling active blocks into the master prompt…<//>
          <//>`
        : html`
          <${M.Stack} gap="md">
            <${M.Group} gap="md" wrap="wrap">
              <${M.Badge} variant="light" color="brand" radius="sm" size="lg" styles=${{ root: { textTransform: 'none', fontWeight: 600 } }}>
                Will publish as <span style=${{ fontFamily: 'var(--mantine-font-family-monospace)', fontWeight: 500 }}>v${version}</span>
              <//>
              <${M.Text} c="dimmed" size="sm" style=${{ fontFamily: 'var(--mantine-font-family-monospace)' }}>${tokens.toLocaleString()} tokens · ${lines} lines<//>
            <//>
            <div style=${{ position: 'relative', border: '1px solid var(--mantine-color-gray-3)', borderRadius: 'var(--mantine-radius-md)', background: 'var(--mantine-color-gray-0)', overflow: 'hidden' }}>
              <div style=${{ maxHeight: '46vh', overflow: 'auto', padding: '16px 18px' }}>
                <${M.Text} component="pre" style=${{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'var(--mantine-font-family-monospace)', fontSize: 12.5, lineHeight: 1.7, color: 'var(--mantine-color-text)' }}>${text}<//>
              </div>
            </div>
            <${M.Group} justify="space-between" wrap="wrap" gap="sm">
              <${M.Group} gap="xs">
                <${M.Button} variant="default" leftSection=${copied ? html`<${Ic.IconCheck} size=${16} />` : html`<${Ic.IconClipboard} size=${16} />`} onClick=${copy}>${copied ? 'Copied' : 'Copy'}<//>
                <${M.Button} variant="default" leftSection=${html`<${Ic.IconDownload} size=${16} />`} onClick=${download}>Download<//>
              <//>
              <${M.Group} gap="xs">
                <${M.Button} variant="subtle" color="gray" onClick=${onClose}>Cancel<//>
                <${M.Button} leftSection=${html`<${Ic.IconRocket} size=${16} />`} onClick=${onPublish}>Publish<//>
              <//>
            <//>
          <//>`}
    <//>`
...
export function Blocks({ notify }) {
  ...
  const nextVersion = D.MASTER_PROMPT_VERSION + 1
  const onCompile = () => {
  ...
  const onPublish = () => { setCompileOpen(false); notify({ color: 'green', title: 'Prompt published', message: `Version ${nextVersion}` }) }
  ...
  return html`
    <${M.Box} data-screen-label="Prompt Studio · Blocks">
      ...
      <${M.Box} p="xl">
        <${M.Stack} gap="lg">
          ...
          ${summaryHidden
            ...
            : html`<${M.Box} style=${{ transition: animate ? 'opacity 200ms' : 'none' }}><${Overview} blocks=${activeBlocks} set=${set} onCompile=${onCompile} notify=${notify} showGuardrail=${showGuardrail} /><//>`}
      ...
      <${CompilePublishModal} opened=${compileOpen} onClose=${() => setCompileOpen(false)} compiling=${compiling} text=${compiledText} set=${set} version=${nextVersion} onPublish=${onPublish} notify=${notify} />
    <//>`
}
