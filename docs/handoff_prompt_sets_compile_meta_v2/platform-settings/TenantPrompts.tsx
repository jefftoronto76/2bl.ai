...
// tenant_id) — see handover §6 for the platform write endpoints this assumes.
//
// Drop at: app/(platform)/platform/settings/TenantPrompts.tsx
...
export function TenantPrompts() {
  ...
  function tenantName(id: string): string {
    return sets.find((s) => s.tenant_id === id)?.tenant_name ?? tenants.find((t) => t.id === id)?.name ?? id
  }
  ...
  function openInComposer(s: PlatformPromptSet) {
    // The Composer reads ?promptSet=<id> on load and preselects it (see handover §6).
