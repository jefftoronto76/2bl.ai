...
Production:
1. **Assets:** place the 11 curated photos in `public/legacy/hero/` (e.g. `hiker.jpg`, `beach.jpg`, …). If marketing needs to swap them without a deploy, back them with a small CMS/Storage map instead — but a static `public/` set is fine and fastest.
