...
const { ChevronRight, Plus, Trash, Alert, Search, X, Dots } = window.Icons;
...
function useToasts() {
  ...
  const show = ({ color = 'green', title, message }) => {
    const id = ++_toastId;
    setItems((prev) => [...prev, { id, color, title, message }]);
...
function NavContent({ active, onNav }) {
  return (
    <>
      <Wordmark />
      <div className="nav-scroll navlinks">
      ...
      <div style={{ padding: 8 }}>
        <button className="userbtn">
          ...
          <span className="u-meta">
            <span className="u-name">Jeff Lougheed</span>
            <span className="u-mail">jeff@2bl.ai</span>
          </span>
          <span className="u-dots"><Dots size={16} /></span>
        </button>
      </div>
    </>
  );
...
function App() {
  ...
  // keep sorted by name within the flat list (query orders by name asc)
  ...
  function handleSave(t) {
    ...
    if (exists) {
      ...
      toasts.show({ color: 'green', title: 'Tenant updated', message: `${t.name} saved.` });
    } else {
      commitTenants([...tenants, t]);
      toasts.show({ color: 'green', title: 'Tenant created', message: `${t.name} is ready.` });
    }
    setModal(null);
  }

  function handleDelete(t) {
    // remove the tenant and any descendants (defensive — DB cascades in prod)
    ...
    toasts.show({ color: 'green', title: 'Tenant deleted', message: `${t.name} was removed.` });
    setModal(null);
  }

  const navTitle = { '/platform/products': 'Products', '/platform/members': 'Members', '/platform/usage': 'Usage' };
  ...
  return (
    <div className="shell">
      ...
      <div className="mobile-header">
        ...
        <span className="mh-title">{({ '/platform/admin': 'Tenants', '/platform/products': 'Products', '/platform/members': 'Members', '/platform/usage': 'Usage' })[route]}</span>
      </div>

      {/* Desktop sidebar */}
      <nav className="navbar desktop">
        <NavContent active={route} onNav={(h) => setRoute(h)} />
      </nav>
      ...
      {drawer && (
        <>
          ...
          <nav className="drawer">
            <NavContent active={route} onNav={(h) => { setRoute(h); setDrawer(false); }} />
          </nav>
        </>
      )}
      ...
      <main className="main">
        <div className="main-inner">
          {route === '/platform/admin' ? (
            ...
            <window.MembersPage notify={toasts.show} />
          ) : (
            <ComingSoon title={navTitle[route]} />
          )}
        </div>
      </main>
