...
| `/api/sessions/[id]` | PATCH | Persists a session's `messages` (+ `visitor_name` when non-empty) and marks it `in_progress`. Scoped by `id` + host-derived `tenant_id` (cross-tenant → 404). |
