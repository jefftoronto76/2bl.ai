...
No files remain in `src/components/` — the directory was removed when Nav moved.
...
- **Chat-UI strangle — widget shell extracted (centralization Step E).** The
  ...
  `useSageParameters` in `services/chat/ui/v1/`. `Nav.tsx` and `SectionProcess.tsx`
  both live in `app/(jefflougheed)/components/` — `src/components/` is gone.
  The `boundaries/element-types` rule is `error` (Step G complete).
