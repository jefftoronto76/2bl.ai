...
| `MessageList` | `app/heirloom/components/chat/MessageList.tsx` | Renders messages (assistant = `Bot`-icon avatar + left bubble, user = right bubble, no avatar) and a bouncing-dots typing indicator while `isLoading`; auto-scrolls to bottom. Assistant prose runs through the marker registry (`createDefaultRegistry()`), so `[BOOKING:]` / `[NAME:]` / `[EMAIL:]` markers are stripped rather than shown as raw text (Heirloom has no booking-card UI, so BOOKING cards are dropped — only prose shows). It also consumes the parsed `markers`: a `[CONTACT:]` marker renders an inline `ContactCard` after the most recent assistant message that carried it (a marker-only message still renders the card even with no prose). Empty assistant bubbles are otherwise skipped, and the engine's `isError` renders an on-brand error bubble. |
| `ContactCard` | `app/heirloom/components/chat/ContactCard.tsx` | Inline Tailwind contact-capture card (Heirloom palette) shown when Sage emits `[CONTACT:]`. Drives a `collect → otp → done` flow. **Collect**: phone (`type="tel"`) + email (`type="email"`), both optional; submit enabled once **either** has a value. **Email-only** → `persistContact` + `captureContact` (no OTP) → warm confirmation. **Phone** → `persistContact` first (so a declined/failed OTP still keeps the contact), then Clerk's classic phone sign-up via `useSignUp` from `@clerk/nextjs/legacy` (`create({ phoneNumber })` → `preparePhoneNumberVerification({ strategy: 'phone_code' })` → inline OTP input → `attemptPhoneNumberVerification({ code })` → `setActive`), then `claimSession()` (server-resolved `user_id`) → warm confirmation. Includes a `#clerk-captcha` mount for bot protection. **Graceful fallbacks**: Clerk not loaded / `create` failure / wrong code / "Skip for now" all conclude warmly with the contact already saved, no account, session anonymous (no error screen). **Returning signed-in users** (`useUser().isSignedIn`, fresh card) are skipped entirely (PR 3 DB recovery handles them). One-shot: `captureContact` sets `state.contact`, so the card never reappears (the completing mount still shows its `done` confirmation). Email-OTP is a future sprint. |
| `Sidebar` | `app/heirloom/components/chat/Sidebar.tsx` | Collapsible nav (`w-12` ↔ `w-64`, toggled via `TOGGLE_SIDEBAR`). Nav items (New Chat / Search / Conversations / Dashboard) + a "Recent" section rendering `recentSessions` from the store (a signed-in user's DB sessions via `GET /api/sessions`); each row calls `loadSession(id)` to load that thread and the active session is highlighted. The block hides when empty, so anonymous visitors (no sessions) see no Recent list. No user-profile footer. |
...
Persistence note: `app/heirloom/lib/chatPersistence.ts` is the pure (no React,
...
newest entry by `updatedAt`. Every entry carries an `updatedAt` ISO timestamp,
...
`app/heirloom/lib/chatPersistence.test.ts`. **This is localStorage only — the
...
| `session.ts` | `handleSessionFinish`, `detectVisitorNameMarker`, `detectVisitorEmailMarker` | Chat `onFinish` detection flows (server-only): token-usage accounting, calendar-offer detection, `[EMAIL:]`-marker capture (`detectVisitorEmailMarker` → lowercase → `isPlausibleEmail` → `persistVisitorEmail` to `chat_sessions.email`), and `[NAME:]`-marker first-name capture (`detectVisitorNameMarker` → titlecase → `isPlausibleName` → persist to `chat_sessions.visitor_name`). The email block runs before the name flow's early returns so both capture in one turn. Name/email capture is **marker-only** — the Haiku extractor was removed in PR #46. No-ops when `sessionId` is null. Consumed by the chat orchestrator (`services/chat/server/index.ts`); imports the `TokenUsage` contract type from `services/chat/server/types`. |
...
| `/api/sessions/[id]` | PATCH | Persists a session's `messages` (+ `visitor_name` when non-empty, + `phone` / `email` from the Heirloom contact card, both written to the `email` column for now — email wins when both present) and marks it `in_progress`. Only supplied fields are written, so a contact-only PATCH (no messages) never clobbers the transcript. Scoped by `id` + host-derived `tenant_id` (cross-tenant → 404). |
| `/api/sessions/[id]/claim` | POST | Links an anonymous session to the now-signed-in user after inline phone/OTP sign-up. Resolves the user **server-side** from the active Clerk session (`ensureClerkUser`, no client-supplied `user_id` → no IDOR) and stamps `chat_sessions.user_id` (`claimSession`, scoped by `id` + host `tenant_id`). 401 when no Clerk session; best-effort from the client (failure leaves the session anonymous). |
...
- Heirloom-specific. Sage emits it as a trigger to ask the visitor for a way to
  ...
  /api/sessions/[id]` that writes the value to `chat_sessions.email` (temporary
  reuse — no phone column yet; **email wins when both are provided**); the
  decline link calls `captureContact(null)`. One-shot per thread.
