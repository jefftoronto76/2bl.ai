...
- **`prompt_sets.is_composer_prompt`'s exclusivity rule changed with no
  DB_CHANGELOG.md entry recording it — a documentation gap in the changelog
  ...
  DB_CHANGELOG.md entry documents the change** — it is recoverable only from
  ...
  DB_CHANGELOG.md entry gets backfilled for this migration, and so nobody
