...
| `tenant_model_config` | id (uuid, primary key), tenant_id (uuid), provider (text default 'anthropic'), model_id (text — primary chat model), model_id_fallback (text — circuit-breaker fallback), max_tokens (integer default 1000), rate_limit_requests_per_hour (integer default 100), created_at (timestamptz), updated_at (timestamptz). Per-tenant model configuration; `services/chat/server/stream.ts` `resolveModelConfig()` reads it when a row exists, falling back to code defaults otherwise. Added 2026-05-23. |
