...
| `ContactCard` | `app/heirloom/components/chat/ContactCard.tsx` | Inline Tailwind contact-capture card (Heirloom palette: `bg-surface`, gold `border-accent/30`, accent submit button) shown when Sage emits `[CONTACT:]`. Phone (`type="tel"`) + email (`type="email"`) inputs — both optional, the visitor provides either or both; submit is enabled once **either** has a value, calling `captureContact({ phone?, email? })` → store `CAPTURE_CONTACT` + a `PATCH /api/sessions/[id]` that writes the value to the `email` column for now (**email wins when both are filled**). Decline link calls `captureContact(null)`. One-shot: returns `null` once `state.contact` is set, so it never reappears. Clerk sign-up is PR 5 — this only captures + persists the contact. |
