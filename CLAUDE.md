...
| `InvitesManager` | `app/admin/invites/InvitesManager.tsx` | Mantine v7 client component on the Invites page. **Gate toggle section (top):** fetches `invite_gate_enabled` from `GET /api/admin/tenant-settings` on mount; PATCHes with `{ invite_gate_enabled }` on Save. Mantine `Switch` ("Require an invite to access Heirloom chat"), dirty tracking, green Save button. When gate is off, the email `TextInput` and "Generate invite" `Button` are disabled. **Invite list section (below gate):** email `TextInput` ("Lock to email", optional), "Generate invite" `Button` (`POST /api/admin/invites`). On success shows a `Modal` with the generated invite URL + copy button. Table of existing invites (token, email, status badge, created/used timestamps) with copy-link and delete `ActionIcon`s per row. Delete opens a confirmation `Modal` and calls `DELETE /api/admin/invites/[id]`. All interactions surface success/error via `@mantine/notifications`. |
...
| Component | File | Purpose |
|-----------|------|---------|
| `HeirloomPage` (server gate) | `app/heirloom/page.tsx` | **Server component** — reads `invite_gate_enabled` from `tenants.settings` JSONB for `HEIRLOOM_TENANT_ID` (default `true` when key absent) and determines `isAuthorized` via two paths: (1) signed-in Clerk user with an `active` members row; (2) valid unused `?invite=TOKEN` query param (checked via `validateInvite()` from `services/invites` — no mark-used at render time). If either path resolves, `isAuthorized = true`. Passes `gateEnabled` and `isAuthorized` to `<HeirloomApp>`. The landing page always renders; the gate only affects the chat widget interior. |
| `HeirloomApp` | `app/heirloom/HeirloomApp.tsx` | `'use client'` shell — accepts `gateEnabled` and `isAuthorized` from the server page, wraps `<ChatProvider gateEnabled={…} isAuthorized={…}>`, and renders the landing page + the chat panel, mounted via **`ChatDrawerV2`** (V2 pass, 2026-06-12): headerless (`showHeader={false}` — ChatHero's `ChatHeader` is the real header), `defaultWidthClassName="w-full max-w-2xl"` preserves the v1 width, `title="Heirloom chat"` preserves the aria-label. Drawer fullscreen state (`isFullScreen`) is a **local `useState` in `HeirloomInner`** (deliberately not in the shell reducer), threaded through `ChatHero` to `ChatHeader`'s expand toggle. Escape key and backdrop click dispatch `CLOSE_CHAT`; the V2 modals/row-menus stop Escape in the capture phase, so one press closes one layer (modal first, panel on the next press). |
| `chatStore` (`ChatProvider`, `useChatStore`, `Message`) | `components/shells/membership/chatStore.tsx` | `useReducer` context. **Gate props**: `ChatProvider` accepts `gateEnabled?: boolean` (default `true`) and `isAuthorized?: boolean` (default `false`); computes `isGated = gateEnabled && !isAuthorized` and exposes it on context — consumed by `ChatHero` to render `GateView` instead of the chat. State: `messages`, `hasStarted`, `isSidebarExpanded`, `isLoading`, `isChatOpen`, `sessionId`. Actions: `SEND_MESSAGE`, `ADD_ASSISTANT_MESSAGE`, `UPDATE_LAST_ASSISTANT`, `SET_LOADING`, `TOGGLE_SIDEBAR` / `SET_SIDEBAR`, `OPEN_CHAT` / `CLOSE_CHAT`, `SET_SESSION_ID`, `HYDRATE` (replaces the conversation wholesale from a recovered localStorage buffer; sets `hasStarted` from whether any messages were restored), `RESET` (clears `messages` / `sessionId` / `isLoading` / `hasStarted` back to the empty-greeting state; preserves UI chrome `isSidebarExpanded` / `isChatOpen`). `sendMessage(content)` is the shared engine's `useChatTurn().send` (PR #44): the provider implements `ChatEngineAccessors` that adapt the reducer (building Heirloom `Message`s with `crypto.randomUUID()` ids + `Date` timestamps, dispatching `SEND_MESSAGE` / `ADD_ASSISTANT_MESSAGE` / `UPDATE_LAST_ASSISTANT` / `SET_LOADING` / `SET_SESSION_ID`), and keeps authoritative `messagesRef` / `sessionIdRef` so the post-stream PATCH reads the full transcript synchronously (the empty assistant message is deferred to the first token to preserve the typing indicator). The context also exposes the engine's `isError`. **localStorage buffering** (best-effort durability in front of the DB): a `persistCurrent` callback writes the live transcript via `bufferThread` (`services/chat/ui/v1/persistence.ts`) on each turn boundary — user send and stream finish (`setStreaming(false)`); **not** per streamed token. `setSessionId` calls `clearDraft()` then re-buffers under the real session key. Once the auth provider loads, a one-shot effect rehydrates the most-recent buffered thread (`findMostRecentThread`) via `HYDRATE` — **signed-in users only**: a signed-out reload starts fresh by design (the buffers stay on disk as the post-sign-in claim index; DB write path untouched). A further effect runs on `pagehide` / `visibilitychange` (`document.visibilityState === 'hidden'`): it flushes the live transcript, then — signed-out only — scrubs all buffered transcripts via `clearTranscripts()` while keeping the session index (so a later sign-in can still claim those sessions; signed-in visitors keep their transcripts as the DB-recovery layer). Post-sign-in, the claim flows (`claimSessionsOnly` / `claimAllSessions`) drop each successfully claimed session's localStorage entry (`clearSession`) plus the unclaimable draft slot, so localStorage is clean once the DB owns the history. **Navigate-away warning**: a `beforeunload` listener (bound once on mount, removed on unmount) triggers the browser's generic leave dialog when a turn is in flight (`isLoadingRef`, a ref mirror of `isLoading` set in `setStreaming`) **or** any conversation exists (`messages.length > 0`). The condition is deliberately broad — anonymous visitors have no cross-device DB recovery yet, so an existing thread is treated as unsaved on leave; tighten to a dirty/confirmed-flush check once signed-in DB recovery is proven. Both `preventDefault()` and `returnValue = ''` are set (Chrome 119+ requires both to show the dialog). **Signed-in DB recovery**: when a Clerk user is signed in (`useUser`), a further effect fetches `GET /api/sessions`, populates `recentSessions` (exposed on the context for the Recent sidebar) and — most-recent-wins — hydrates from the newest DB session only when its `updated_at` is strictly newer than the local buffer's `updatedAt` (so a fresher local thread is never clobbered). The Recent list is also **reactive within a visit**: on each turn finish (`isStreaming` true→false) while signed in, the live thread is upserted into `recentSessions` locally (newest-first, keyed on `sessionId`) — deliberately no `GET /api/sessions` refetch there, since the engine's transcript PATCH is fired fire-and-forget *after* streaming ends and a refetch would race it; the DB fetch on the next load reconciles. `loadSession(id)` (also on the context) loads a previously-fetched session into the conversation via `HYDRATE`. Anonymous users skip this entirely (localStorage only). `newChat()` (also on the context, backing the Sidebar "New Chat" button) captures the active `sessionIdRef` value, clears the engine refs (`messagesRef` / `sessionIdRef` / `assistantPendingRef` / `isLoadingRef`), drops the active thread's localStorage entries — `clearDraft()` for the pre-session draft slot **and** `clearSession(clearedSessionId)` for the current session-keyed entry — then dispatches `RESET`. Clearing the **session** entry (not just the draft) is what actually stops the next mount's rehydration (`findMostRecentThread`) from reloading the just-cleared conversation on refresh: once a conversation has had a completed turn it lives under its real session key, so `clearDraft()` alone left it behind to be re-hydrated (the bug this fixed). History under **other** real session keys + `recentSessions` are preserved (other threads stay loadable from Recent; DB rows are untouched), and the next turn lazily creates a fresh session so the chat returns to the empty greeting. Contact capture is no longer a client concern — phone/email are captured server-side from the visitor's message by the watcher in `services/crm/session.ts` (the `ContactCard` + `CAPTURE_CONTACT` flow was removed). |
| `ChatHero` | `components/shells/membership/ChatHero.tsx` | Panel body: **`SidebarV2`** (V2 pass — the v1 `Sidebar` import was swapped) + a column with `ChatHeader`, then the main content area. Accepts optional `isFullScreen` / `onToggleFullScreen` props (from `HeirloomInner`) passed through to `ChatHeader`. Owns the V2 sidebar wiring: `stories` is **ephemeral client `useState`** (no stories backend yet — created stories demo the flow for the session; rows are inert), `WRITING_PROMPTS` is a static 4-prompt client array (placeholder copy, review before launch) whose `onSelectPrompt` sends into the conversation guarded by `isGated`/`isLoading`; Uploads / Share / Search / Invite / kebab handlers are deliberately not passed (stubs). Mounts `BeginStoryModal` as the last section child — its `absolute inset-0` overlay resolves to the drawer's `relative` body. **When `isGated`** (from `useChatStore`): renders `<GateView />` occupying the full content area — no composer. **When not gated**: renders `MessageList` (once `hasStarted`), else the empty-state greeting **"What's a story worth keeping?"**, followed by `ChatInput` + `SaveChatCTA`. **iOS keyboard handling**: `useKeyboardViewport({ active: state.isChatOpen, lockBodyScroll: true })` — surface shrinks to `vv.height`px when keyboard is open. |
| `GateView` | `components/shells/membership/GateView.tsx` | Rendered by `ChatHero` when `isGated`. Three stages: **gate** — "By invitation only." heading + "Claim a free membership" button; **signup** — `<MagicLinkCard reason="claim_membership" onSuccess={handleClaimSuccess}>` inline (no navigation); **claimed** — "You're on the list." confirmation. `handleClaimSuccess` calls `claimCurrentSession()` then `POST /api/heirloom/members/claim` (fire-and-forget) to create a `pending` membership, then advances to the `claimed` stage. Tailwind + Heirloom tokens. No Mantine. |
| `ChatHeader` | `components/shells/membership/ChatHeader.tsx` | "Your Story" dropdown + right cluster: **Share `IconButton` (deliberately inert this pass — `aria-disabled`, no share backend)**, **fullscreen toggle (`Maximize2`/`Minimize2`, renders only when `onToggleFullScreen` is provided)**, Account dropdown (sign-in entry — `logAuthStep` telemetry + `setOAuthInProgress`), Close (dispatches `CLOSE_CHAT`). |
...
routes and the public mark-used route as thin consumers.

| File | Exports | Purpose |
|------|---------|---------|
| `invites.ts` | `listInvites`, `createInvite`, `validateInvite`, `markInviteUsed`, `deleteInvite` (+ `InviteRow`, `InviteResult`) | Data-access against the `invites` table via service-role client. Token generation uses `crypto.randomBytes(24).toString('base64url')` (32-char URL-safe string). `validateInvite` checks token exists + `used_at IS NULL` + expiry. `markInviteUsed` sets `used_at = now()` with an `is('used_at', null)` guard against double-use. All write functions return discriminated `InviteResult<T>`. |
| `index.ts` | barrel | Re-exports the public surface above. |
...
| `index.ts` | barrel | Re-exports the type contracts + the registry runtime (`createMarkerRegistry`, `createDefaultRegistry`, `BOOKING_MARKER`, `NAME_MARKER`, `EMAIL_MARKER`, `PHONE_MARKER`, `ACCOUNT_CREATE_MARKER`). `useChatTurn` is imported directly from its module, not the barrel. |

**`core/` — session + keyboard infrastructure**

| File | Exports | Purpose |
|------|---------|---------|
| `core/store.ts` | `createChatSessionStore`, `ChatSessionStore`, `ChatSessionState`, `HydrateInput` | Pure, framework-agnostic conversation store backed by `zustand/vanilla`. Holds `messages`, `sessionId`, `isStreaming`, `isError`, `mode`. No shell/presentation state. |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/invites` | GET | Returns all `invites` rows for the authenticated tenant, newest first. 401 when `getAuthContext()` fails. |
| `/api/admin/invites` | POST | Creates a new invite. Accepts optional `{ email: string }` (stored lowercased, trimmed). Generates a URL-safe token via `createInvite()`. Returns the new row with 201. |
| `/api/admin/invites/[id]` | DELETE | Deletes the invite by `{ id, tenant_id }` (cross-tenant IDOR guard). 401 when `getAuthContext()` fails, 500 on error. |
| `/api/heirloom/invites/use` | POST | Public route. Accepts `{ token: string }`, calls `markInviteUsed(token)`. Called client-side after sign-up via `claimAllSessions` in `chatStore.tsx`. Returns 404 if the token is missing, already used, or not found. Fire-and-forget — a failed mark-used never blocks the signed-up user. |
...
| `invites` | id (uuid PK), token (text unique — 32-char base64url, generated server-side), tenant_id (uuid FK → tenants), created_by (uuid FK → users), email (text nullable — optional lock to a specific person), used_at (timestamptz nullable — set on first use; null = pending), expires_at (timestamptz nullable), created_at (timestamptz NOT NULL default now()). RLS enabled (all access via service-role client). Consumed by the Heirloom invite gate (`app/heirloom/page.tsx`) and the admin invite management page (`app/admin/invites/`). |
...
| `members` | id (uuid, PK), clerk_id (text, unique — the Clerk user id, e.g. `user_...`; renamed from `clerk_user_id` 2026-06-10 to match `users`), tenant_id (uuid, FK → tenants — scopes the member to a product tenant), name (text, nullable — display name supplied during sign-up; written via `syncMember` from `POST /api/members/sync`), email (text, nullable — synced from Clerk on auth), phone (text, nullable — synced from Clerk on auth), status (text, default 'active'), created_at (timestamptz), updated_at (timestamptz). Heirloom membership record created/updated on each authentication via `syncMember` (`services/auth/sync-member.ts`). Heirloom tenant_id: `20767f1d-1148-4e43-ab73-f6da88f0ac56`. |
