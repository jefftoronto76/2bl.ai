...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/tenant-settings` | GET | Returns `{ chat_in_progress_idle_seconds, chat_active_idle_seconds, invite_gate_enabled, default_primer }` for the authenticated tenant. `invite_gate_enabled` is read from `tenants.settings` JSONB (default `true` when key absent). `default_primer` is read-only — echoed straight from the `tenants` row (see that schema row's note), not an input field on this route. 401/404/500 as above. Consumed by `ChatThresholds` (Settings page) and `InvitesManager` (Invites page). |
