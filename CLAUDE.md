...
| `ContactCard` | `app/heirloom/components/chat/ContactCard.tsx` | Inline Tailwind contact-capture card (Heirloom palette: `bg-surface`, gold `border-accent/30`, accent submit button) shown when Sage emits `[CONTACT:]`. A `type="tel"` phone input + submit (`captureContact(phone)` → store `CAPTURE_CONTACT` + a `PATCH /api/sessions/[id]` writing the phone to the `email` column for now) and a decline link (`captureContact(null)`). One-shot: returns `null` once `state.contact` is set, so it never reappears. Light client-side phone check (7–15 digits). Clerk sign-up is PR 5 — this only captures + persists the number. |
...
| `sessions.ts` | `createSession`, `updateSession`, `listSessions` (+ `SessionResult`, `SessionUpdateInput`, `ChatSessionSummary`) | Visitor session writes + signed-in reads. Server-role client, scoped by both `id` AND host-derived `tenant_id` (cross-tenant IDOR guard; cross-tenant id → 404). `createSession` takes an optional `userId` and writes `chat_sessions.user_id` when present (anonymous → null). `listSessions(tenantId, userId)` returns the user's sessions newest-first, scoped by `user_id` + `tenant_id` (`messages` typed as opaque `unknown` jsonb — the CRM layer does not depend on the UI message shape). Backs `POST` / `PATCH /api/sessions/[id]` / `GET /api/sessions`, which are thin (tenant + user resolution + parsing + response mapping). |
...
| `/api/sessions/[id]` | PATCH | Persists a session's `messages` (+ `visitor_name` when non-empty, + `phone` from the Heirloom contact card written to the `email` column for now) and marks it `in_progress`. Only supplied fields are written, so a contact-only PATCH (phone, no messages) never clobbers the transcript. Scoped by `id` + host-derived `tenant_id` (cross-tenant → 404). |
...
- Heirloom-specific. Sage emits it as a trigger to ask the visitor for a phone
  number; the field is usually empty (the visitor types their number into the
  card, Sage doesn't have it). **Client render**: the Heirloom `MessageList`
  detects the parsed `CONTACT` marker and renders an inline `ContactCard`
  (`app/heirloom/components/chat/ContactCard.tsx`) after the most recent
  assistant message carrying it. Submit calls `captureContact(phone)` →
  `CAPTURE_CONTACT` + a `PATCH /api/sessions/[id]` that writes the phone to
  `chat_sessions.email` (temporary reuse — no phone column yet); the decline
  link calls `captureContact(null)`. One-shot per thread.
...
- Emission requires the Heirloom `master_prompt` to instruct Sage to emit
  `[CONTACT:]` (a follow-up; the fallback `DEFAULT_SYSTEM_PROMPT` does not).
- Clerk phone/OTP sign-up on top of this capture is **PR 5** — this marker only
  captures + persists the number.
