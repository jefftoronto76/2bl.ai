...
| `compile.ts` | `compilePrompt(tenantId, promptSetId, note)` | Compiles active blocks (identity → knowledge → guardrail → process → output_format; `order`-aware) into `compiled_prompts`, archiving the prior version (content + its own release-note fields) to `compiled_prompts_history`. `note` (`ReleaseNote`, July 2026 — required) is written onto the row this call creates/overwrites (`release_summary`/`release_why`/`release_changed_block_ids`); the *previous* note rides into history automatically on the archive step, one publish later. **Activation (July 2026, single-live-per-type):** Publish is the only activation event, and it is unconditional — there is no "compile without activating." Every call resolves the target prompt_set's `prompt_type_id` (never client-supplied — read fresh from `prompt_sets`; absent `promptSetId` means the legacy no-set default slot, type always null), clears any *other* row/set currently `status='live'` for that same `(tenant_id, prompt_type_id)` slot in **both** `compiled_prompts` and `prompt_sets` (clear-then-set, mirrors the `is_composer_prompt` pattern in `app/api/platform/settings/master-prompt/route.ts`), then writes the new compiled row as `status='live'` and flips the target `prompt_sets.status` to `'live'` too — regardless of the set's status going in (publishing a draft activates it). The two partial unique indexes (`compiled_prompts_single_live_typed_idx` on `(tenant_id, prompt_type_id) WHERE status='live' AND prompt_type_id IS NOT NULL`, and `compiled_prompts_single_live_untyped_idx` on `(tenant_id) WHERE status='live' AND prompt_type_id IS NULL` — NULLs aren't equal to each other in a unique index, so the untyped/default slot needs its own index) are the backstop, same reasoning as the composer pattern: no true transaction here, so clear-then-write ordering minimizes the race window and the index catches what slips through — a `23505` on the final write surfaces as a friendly 409 ("Another publish for this prompt type just landed — please retry"), not a raw 500. Enforced at the write path only: `getSystemPrompt` (`compiler.ts`) is unchanged, still reads the tenant's highest-version row with no type filtering — making it slot-aware is separate, still-open work (see Known Gaps). Requires `compiled_prompts.status` / `.prompt_type_id` (Jeff, Studio — not yet applied as of 2026-07-27; see DB_CHANGELOG once run). Backs `POST /api/admin/prompt/compile` (also now returns 404 if the target prompt_set doesn't exist, 409 on the activation race). |
...
- **`getSystemPrompt` is not type-aware — single-live-per-type (2026-07-27)
  constrains Publish but not runtime reads.** `services/prompt/compiler.ts`'s
  `getSystemPrompt` still reads the tenant's highest-`version` `compiled_prompts`
  row with no `prompt_type_id` filtering at all. The new partial unique indexes
  (`compiled_prompts_single_live_typed_idx` / `_untyped_idx`, see the
  `compiled_prompts` schema row and `compile.ts`) guarantee at most one live row
  per `(tenant_id, prompt_type_id)` slot, but do nothing to make the runtime read
  pick the *correct* slot once a tenant has more than one live type (e.g. a live
  Base and a live Sales prompt for the same tenant) — whichever has the higher
  `version` number wins, regardless of type. This "worked" before only because
  every tenant happened to have exactly one compiled row. Making `getSystemPrompt`
  filter by the runtime-relevant type (Base, or whatever a session's `mode`/
  context calls for) is separate, still-open work — do not treat the Publish-side
  constraint as having fixed this.
