...
| `InviteGateSettings` | `app/admin/settings/InviteGateSettings.tsx` | Mantine v7 client component rendered as the third section on the Settings page. Owns the section header "Invite Gate" and a `Card` with a `Switch` ("Require membership to access chat"). Fetches `invite_gate_enabled` from `GET /api/admin/tenant-settings` on mount; PATCHes with `{ invite_gate_enabled }` on Save. Dirty tracking — Save button enabled only when the switch value differs from the last-saved snapshot. Surfaces success/error via `@mantine/notifications`. |
...
| Page | File | Purpose |
|------|------|---------|
| Settings | `app/admin/settings/page.tsx` | Tenant configuration. Renders three sections: Parameters (`SageParameters`), Chat Thresholds (`ChatThresholds`), and Invite Gate (`InviteGateSettings`). |
...
| Component | File | Purpose |
|-----------|------|---------|
| `HeirloomPage` (server gate) | `app/heirloom/page.tsx` | **Server component** — reads `invite_gate_enabled` from `tenants.settings` JSONB for `HEIRLOOM_TENANT_ID` (default `true` when key absent) and checks whether the signed-in Clerk user has an `active` members row. Passes `gateEnabled` and `isAuthorized` to `<HeirloomApp>`. The landing page always renders; the gate only affects the chat widget. |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/tenant-settings` | GET | Returns `{ chat_in_progress_idle_seconds, chat_active_idle_seconds, invite_gate_enabled }` for the authenticated tenant. `invite_gate_enabled` is read from `tenants.settings` JSONB (default `true` when key absent). 401/404/500 as above. Consumed by `ChatThresholds` and `InviteGateSettings` on the Settings page. |
