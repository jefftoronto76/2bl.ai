...
intentionally stays in `app/globals.css` for now — it is coupled to the
`Chat`/`Hero` components that don't move until Phase 3. The `nav-chat-*` classes
also still live here but are now **orphaned** — the refreshed `Nav` (Book / Labs
/ Share) styles itself with inline styles and no longer references them.
...
- **Public assets** live in `public/sage/jefflougheed/` and are referenced as
  ...
  internal icon `src`s point at `/sage/jefflougheed/favicons/…`. The favicons
  use the inkwell palette — the bone "JL" mark (`#EAE7DC`) on a dark-slate
  ground (`#182029`) — across `favicon.svg`, `favicon.ico`, and the PNG set.
...
| `Hero` | `src/components/Hero.tsx` | **Chat-first hero** — an eyebrow ("Coach · Operator · Builder"), the landing headline ("Hi, I'm Jeff." — "Jeff" wrapped in `<em>` but rendered accent-green and **non-italic** via `.hero h1 em { font-style: normal }`), a lede ("…think clearly, develop their capabilities, and grow in ways that last." — plain copy, no highlight span), and `sage-line` sit above an **inline chat surface** (composer + conversation canvas + suggestion chips) in the same `#hero` section. It is a standalone inline chat that **shares session state with the Chat overlay** via `useSageStore` (the two surfaces drive one conversation) but does **not** use `expand()`. The turn engine is the shared `useChatTurn` hook (`services/chat/ui/v1/`), wired via a `useMemo`'d `ChatEngineAccessors` that reads/writes live store state through `useSageStore.getState()`; `submit()`, Enter-to-send (`onKey`), and the suggestion-chip `handleChipClick` all call `turn.send(text)`, the error block's Retry calls `turn.retry()`, and `turn.isError` drives that block. `isStreaming` (store) drives the three-dot `sage-pulse` indicator and disables the composer/chips. Messages render like the overlay: visitor as right-aligned italic Playfair `.sage-visitor-msg`, assistant via `SageReply` (booking cards parsed by `parseBookingCards`, params from `useSageParameters`). `isEngaged = messages.length > 0 && conversationVisible` toggles the `.stage` → `.stage engaged` class; a `close-x` button collapses the canvas (`conversationVisible`), and the 5 suggestion chips render only when not engaged. On mount it registers the textarea via `setComposerRef` and runs `detectModeFromLocation()` — `?mode=question` (hash-query or top-level search) → `setMode('question')` + focuses the composer. **iOS keyboard handling**: a `visualViewport` listener mirrors `vv.height` / `vv.offsetTop` onto the `.chat-surface` via the `--kb-surface-h` / `--kb-surface-y` CSS vars and flips `.chat-surface--kb` when `keyboardOpen` (mobile only — `vv.height` never drops on desktop, so layout is untouched); **no body scroll-lock** (deliberate — `position: fixed` breaks iOS keyboard detection, so the fixed `.chat-surface--kb` overlay masks the page instead). |
| `Nav` | `src/components/Nav.tsx` | Top fixed navigation (`height 60px`). Transparent at the top → `rgb(var(--color-bg) / 0.96)` + `blur(12px)` + bottom border once `scrollY > 60` or the mobile drawer is open. Left: a Playfair wordmark (`.nr-brand`, 17px desktop / 14px mobile) reading "Performance-Driven, Heart-Led" and linking to `#hero`. Right: three links from a `LINKS` array, rendered as muted uppercase mono items (`0.18em` tracking) — **Book** (`kind: 'scroll'` → `preventDefault` then smooth-scrolls to `#how-it-works`), **Labs** (`kind: 'external'` → opens `https://www.2bl.ai` in a new tab with `rel="noopener noreferrer"`), and **Share** (`kind: 'share'` → opens `ShareModal`). **Desktop** shows the links inline (`.nr-desktop-links`); below 768px they collapse into a hamburger button (`.nr-mobile-menu-btn`, animated 3-bar ↔ X) toggling a fixed dropdown drawer that renders the same three links (larger, row-bordered). `ShareModal` (`app/(jefflougheed)/components/ShareModal.tsx`) is a `react-share`-backed dialog — LinkedIn / Twitter-X / WhatsApp / Email share buttons plus a Copy-link row; bottom-sheet on mobile, centered on desktop, `role="dialog"`/`aria-modal`, Escape-to-close with focus moved into the panel. Styling is inline `style` objects + a small injected `<style>` (`.nr-brand` sizing + responsive show/hide). The Nav no longer consumes `useSageStore` or the legacy `.nav-chat-*` classes — those classes still exist in `app/globals.css` but are now orphaned. |
