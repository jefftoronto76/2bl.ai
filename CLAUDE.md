...
| `useIsMobile` | `useIsMobile.ts` | Headless `matchMedia('(max-width: 768px)')` hook (no JSX) — SSR-safe (`false` until mounted), re-evaluates on a `change` listener. Consumed by `WidgetShellHero` (`components/shells/widget/WidgetShell.tsx`) to gate the mobile first-send keyboard-close + full-screen-settle sequencing (see the `WidgetShellHero` row under Public Site). Mirrors the `768px` breakpoint already baked into `app/(jefflougheed)/globals.css`'s `.stage.engaged` mobile rules — added rather than pulling in `@mantine/hooks`' `useMediaQuery` (the Heirloom membership shell's equivalent) to keep the Tailwind-only public bundle Mantine-free. |
