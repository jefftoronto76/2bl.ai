...
| `chatStore` (`ChatProvider`, `useChatStore`, `Message`) | `app/heirloom/components/store/chatStore.tsx` | `useReducer` context. State: `messages`, `hasStarted`, `isSidebarExpanded`, `isLoading`, `isChatOpen`, `sessionId`. Actions: `SEND_MESSAGE`, `ADD_ASSISTANT_MESSAGE`, `UPDATE_LAST_ASSISTANT`, `SET_LOADING`, `TOGGLE_SIDEBAR` / `SET_SIDEBAR`, `OPEN_CHAT` / `CLOSE_CHAT`, `SET_SESSION_ID`. Exposes `sendMessage(content)`, which wires the full session lifecycle (PR #39): on the first send it POSTs `/api/sessions` to create a session (tracking the id locally for the turn since the reducer dispatch is async), POSTs `{ messages, session_id }` to `/api/sage`, streams deltas via `readSageStream` (dispatching `ADD_ASSISTANT_MESSAGE` on the first token then `UPDATE_LAST_ASSISTANT`), and once the stream settles PATCHes `/api/sessions/[id]` with the final transcript (`visitorName: null`). Errors surface as an on-brand assistant message. |
...
**Client parsing** (`parseBookingCards` in `src/components/sage/parseBookingCards.ts`):
