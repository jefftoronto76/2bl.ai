...
| `compile.ts` | `compilePrompt(tenantId, promptSetId, note)` | Compiles active blocks (identity → knowledge → guardrail → process → output_format; `order`-aware) into `compiled_prompts`, archiving the prior version (content + its own release-note fields) to `compiled_prompts_history`. `note` (`ReleaseNote`, July 2026 — required) is written onto the row this call creates/overwrites (`release_summary`/`release_why`/`release_changed_block_ids`); the *previous* note rides into history automatically on the archive step, one publish later. **Activation (July 2026, single-live-per-type):** Publish is the only activation event, and it is unconditional — there is no "compile without activating." Every call resolves the target prompt_set's `prompt_type_id` (never client-supplied — read fresh from `prompt_sets`; absent `promptSetId` means the legacy no-set default slot, type always null), clears any *other* row/set currently `status='live'` for that same `(tenant_id, prompt_type_id)` slot in **both** `compiled_prompts` and `prompt_sets` (clear-then-set), then writes the new compiled row as `status='live'` and flips the target `prompt_sets.status` to `'live'` too — regardless of the set's status going in (publishing a draft activates it). The two partial unique indexes (`compiled_prompts_single_live_typed_idx` on `(tenant_id, prompt_type_id) WHERE status='live' AND prompt_type_id IS NOT NULL`, and `compiled_prompts_single_live_untyped_idx` on `(tenant_id) WHERE status='live' AND prompt_type_id IS NULL` — NULLs aren't equal to each other in a unique index, so the untyped/default slot needs its own index) are the backstop: no true transaction here, so clear-then-write ordering minimizes the race window and the index catches what slips through — a `23505` on either compiled_prompts write, or on the final `prompt_sets` activation write (July 2026 — this one lacked the catch until then), surfaces as a friendly 409 ("Another publish for this prompt type just landed — please retry"), not a raw 500. **Composer-family exclusivity (July 2026), a SECOND independent rule:** when the target `prompt_set` has `is_composer_prompt=true`, an additional clear step retires any other row with `is_composer_prompt=true AND status='live'` — platform-wide, not scoped to `(tenant_id, prompt_type_id)`, since two composer sets in different type slots would never collide in the type-based clear above. Backed by `prompt_sets_single_composer_idx` (`WHERE is_composer_prompt=true AND status='live'`). This is the **only** activation path for a composer prompt set — the old Platform Settings "Save" pointer is retired, see Known Gaps. Enforced at the write path via the partial unique indexes above; on the read path, `getSystemPrompt` (`compiler.ts`) filters to `status = 'live'` as of 2026-07-28 but still has no `prompt_type_id` filtering — a tenant with more than one live type resolves by highest `version` among all its live rows, not necessarily the runtime-relevant type; making it slot-aware is separate, still-open work (see Known Gaps). Requires `compiled_prompts.status` / `.prompt_type_id` (Jeff, Studio — applied by 2026-07-28). Backs `POST /api/admin/prompt/compile` (also now returns 404 if the target prompt_set doesn't exist, 409 on the activation race). |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/prompt-sets` | GET | Returns the authenticated tenant's `prompt_sets` rows as an **unwrapped** `PromptSet[]` (`id, tenant_id, label, description, status, is_composer_prompt, is_default, prompt_type_id, version, created_at, updated_at`), ordered `created_at desc`. **Excludes composer-family rows** (`.not('is_composer_prompt', 'is', true)`, added July 2026) — this is the ordinary tenant Settings screen; composer sets are managed exclusively from Platform Settings + Blocks, even for the SBL tenant itself (expect zero rows there until SBL has an ordinary prompt_set of its own). Consumed by the Settings `PromptSets` panel and the Composer picker (`app/admin/prompt-builder/page.tsx`). 401 on auth failure, 500 on DB error. |
