...
- **`app/layout.tsx`** — the shared root layout. `<ClerkProvider afterSignOutUrl="/">` is
  ...
  Exports the `Viewport` config (`interactiveWidget: 'resizes-content'`,
  `viewportFit: 'cover'`) — the only `viewport` export in the app, so
  jefflougheed and Heirloom both inherit it (neither layout overrides it).
  `viewportFit: 'cover'` is required for `env(safe-area-inset-bottom)` to
  resolve to a non-zero value on notched iOS devices; every chat surface's
  safe-area padding depends on it. It imports the global base layer
  (`app/globals.css` — reset + shared component
...
- **`app/admin/` + `app/(platform)/`** — the Mantine admin and 2BL platform
  ...
  picker that flips `prompt_sets.is_composer_prompt`; page + `MasterPromptPicker` co-located
  there, Mantine).
...
`Hero.tsx`, `Chat.tsx`, and `sage/*` (`SageReply`, `BookingCard`,
`markdownComponents`) now live in **`components/shells/widget/`** (app-importable
shared presentation), with the headless pieces (`useWidgetShell`,
...
| `ChatInput` | `components/shells/membership/ChatInput.tsx` | Auto-growing textarea, Enter-to-send (Shift+Enter newline), `ArrowUp` send button (disabled when empty or loading). Calls `sendMessage`. No AI disclaimer. Outer composer container carries `padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 12px)` (normal flow, not `position: fixed` — `ChatHero`'s `useKeyboardViewport` call already height-clamps the containing section to the visual viewport) so the composer clears the home-indicator safe area on notched iOS devices; depends on `viewport-fit: cover` (`app/layout.tsx`) to resolve to a non-zero value. |
...
| `feedback.ts` | `upsertFeedback`, `listFeedback`, `resolveMemberId` (+ `MessageFeedbackRow`, `UpsertFeedbackInput`, `FeedbackResult`) | `message_feedback` reads/writes for the visitor-facing thumbs rating UI. `upsertFeedback` upserts on `(session_id, message_index)`, tenant-scoped (cross-tenant id matches no row, same IDOR guard as `updateSession`); `rating: null` clears the rating + tags/detail (toggle-off). `listFeedback` hydrates a session's ratings on mount. `resolveMemberId` prefers the server-verified Clerk identity over a client-supplied member id, falling back to the client value when there's no Clerk session. Backs `POST`/`GET /api/sessions/[id]/feedback`; bulk-read for the admin surfaces happens separately in `inbound.ts` (not through `listFeedback`, which is single-session). |
...
| `persistence.ts` | `bufferThread`, `clearDraft`, `clearSession`, `readThread`, `readIndex`, `findMostRecentThread`, `sessionKey` (+ `PersistedMessage`, `PersistedThread`, `ThreadIndexEntry`, `INDEX_KEY`, `DRAFT_KEY`, `DRAFT_ID`) | Pure (no React, no store) localStorage buffer for the Heirloom chat — see the Heirloom "Persistence note" above for behavior. Storage keys are `heirloom:chat:v1:*` (unchanged). Moved here from `app/heirloom/lib/chatPersistence.ts` in centralization Step B. Unit-tested in `persistence.test.ts`. |
...
| `core/useChatSession.ts` | `useChatSession`, `ChatSession`, `ChatSessionConfig` | Core hook: resolves the backing store (singleton if `instanceKey` provided, otherwise ref-local isolated), builds `ChatEngineAccessors`, calls `useChatTurn` once, exposes `send` / `retry` / `setMode` / `hydrate` / `reset` plus the full `ChatSession` state. One provider → one engine. |
...
| `core/useKeyboardViewport.ts` | `useKeyboardViewport`, `UseKeyboardViewportOptions`, `UseKeyboardViewportReturn`, `KeyboardViewportMeasurement`, `KeyboardViewportState` | Shared iOS visual-viewport hook. Listens to `visualViewport` resize/scroll events; returns `{ height, offsetTop, keyboardOpen, sync }`. Every `sync()` call also writes `visualViewport.height` to a global `--vvh` CSS custom property on `document.documentElement` — unconditional on the calling instance's `trackViewport`/`onViewportChange` config, so any surface's CSS can key off `var(--vvh)` for keyboard-affected sizing as long as at least one mounted instance is actively tracking (the Telegram-pattern convergence: no surface should size itself off `dvh` alone). Optional `lockBodyScroll` (freezes `document.body` while active, restores scroll position on deactivate), `trackViewport: false` (scroll-lock only, no VV listeners — no current call site sets this, kept for a future scroll-lock-only consumer), and `onViewportChange` callback for a surface with a genuine per-instance need the global var doesn't cover (e.g. Hero's offsetTop-driven `translateY` — see below) rather than a reflow-free CSS-var write. **Centralisation**: for the widget-shell surfaces, the hook call itself lives inside `components/chat/ChatThread.tsx` (its `keyboardConfig` prop) rather than being called directly by `WidgetShellChat`/`WidgetShellHero`. Heirloom's `ChatHero.tsx` keeps its own direct call — see that row below for why. SSR-safe; no-ops on browsers without the VisualViewport API and when `active: false`. See `docs/chat-shells.md` §3 for per-surface wiring details (not yet updated for this centralisation pass). |
...
| `/api/admin/blocks/[id]` | PATCH | Updates block `status`, `title`, `body`, or `order`. Validates status against `'active' \| 'disabled' \| 'deleted'`; `title` must be non-empty after trim (400 if empty); `order` must be an integer. Keeps the legacy `active` boolean in sync with `status` so the Composer GET doesn't surface disabled or deleted blocks. Tenant-scoped via `.eq('tenant_id', authCtx.tenant_id)`. |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/prompt-sets` | GET | Returns the authenticated tenant's `prompt_sets` rows as an **unwrapped** `PromptSet[]` (`id, tenant_id, label, description, status, is_composer_prompt, is_default, prompt_type_id, version, created_at, updated_at`), ordered `created_at desc`. Consumed by the Settings `PromptSets` panel and the Composer picker (`app/admin/prompt-builder/page.tsx`). 401 on auth failure, 500 on DB error. |
| `/api/admin/prompt-sets` | PATCH | Upsert. Body `{ id?, label, description, status, prompt_type_id }`. With `id` → tenant-scoped update (404 if the id isn't in the tenant); without `id` → insert (`version`/`is_composer_prompt`/`is_default` from DB defaults). `version`/`is_composer_prompt`/`is_default`/timestamps are server-owned and never written. `prompt_type_id` is forced null unless `status === 'live'`, and when non-null it is validated to belong to the tenant's `prompt_types` (400 otherwise). A live set with no `prompt_type_id` is rejected (400). Writes `PROMPT_SET_UPSERT` audit. |
